"""
Orbit Wars - v132: v131 with comet prediction and timed capture
Key changes:
- Comet spawn prediction (turns 50, 150, 250, 350, 450 every 100 turns)
- Launch ships to arrive at comets when they spawn
- Keep ships to evacuate within 18 turns
- Continue v131 continuous fleet launching improvements
"""
import os
...
def agent(obs):
    ...
    for src in my:
        ...
        if phase == 'smash' and not smash_fallback:
            ...
            send = int(available * (_SEND_AGGRESSIVE_4P if num_players >= 4 else 0.4))
            send = max(send, ships_needed_for_takeover(tgt['ships'], tgt['prod'], tt, tgt['owner']))
