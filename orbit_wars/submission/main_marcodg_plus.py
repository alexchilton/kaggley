...
# Comet urgency (marcodg_plus addition)
COMET_SPAWN_TURNS = frozenset({50, 150, 250, 350, 450})
COMET_URGENCY_WINDOW = 12
COMET_URGENCY_BOOST = 1.15

# 4-player opening (marcodg_plus addition)
FP_RUNAWAY_RATIO = 1.18
FP_RUNAWAY_TARGET_BONUS = 1.35
FP_OPENING_LIMIT = 24
FP_EARLY_NEUTRAL_BOOST = 1.25

# Denial scoring (marcodg_plus addition)
DENIAL_VALUE_BOOST = 1.45
DENIAL_PROACTIVE_PROD_THRESHOLD = 3
...
def aim_with_prediction(src, target, ships, initial_by_id, ang_vel, comets, comet_ids):
    ...
    for _ in range(5):
        ...
        if (
            ...
            # Post-convergence sun safety check
            lx, ly = launch_point(src.x, src.y, src.radius, next_est[0])
            if segment_hits_sun(lx, ly, ntx, nty):
                return search_safe_intercept(src, target, ships, initial_by_id, ang_vel, comets, comet_ids)
    ...
    if final_est is None:
        return search_safe_intercept(src, target, ships, initial_by_id, ang_vel, comets, comet_ids)
    # Post-convergence sun safety check
    lx, ly = launch_point(src.x, src.y, src.radius, final_est[0])
...
def target_value(target, arrival_turns, mission, world, modes, policy):
    ...
    if target.id in world.comet_ids:
        ...
        # Comet urgency: boost near known spawn windows
        if any(abs(world.step - st) <= COMET_URGENCY_WINDOW for st in COMET_SPAWN_TURNS):
            value *= COMET_URGENCY_BOOST
    ...
    # 4P opening: runaway leader targeting + early neutral boost
    if modes.get("fp_runaway_owner") is not None and target.owner == modes["fp_runaway_owner"]:
        value *= FP_RUNAWAY_TARGET_BONUS
    if modes.get("fp_early_neutral") and target.owner == -1:
        value *= FP_EARLY_NEUTRAL_BOOST

    # Denial scoring: boost high-prod neutrals we can reach before enemies
    if target.owner == -1 and target.production >= DENIAL_PROACTIVE_PROD_THRESHOLD and world.enemy_planets:
        ...
        if my_dist < enemy_dist and enemy_dist < 30:
            value *= DENIAL_VALUE_BOOST
...
def four_player_opening_adjust(world, modes):
    """4P opening: early neutral boost + runaway leader targeting."""
    if not world.is_four_player or world.step > FP_OPENING_LIMIT:
        return
    strength = {}
    for p in world.planets:
        if p.owner >= 0:
            strength[p.owner] = strength.get(p.owner, 0) + p.ships + p.production * 10
    standings = sorted(strength.items(), key=lambda x: x[1], reverse=True)
    if len(standings) < 2:
        return
    leader, lead_str = standings[0]
    second_str = standings[1][1]
    if leader != world.player and lead_str > second_str * FP_RUNAWAY_RATIO:
        modes["fp_runaway_owner"] = leader
    if world.step < 16:
        modes["fp_early_neutral"] = True
...
def plan_moves(world, deadline=None):
    ...
    four_player_opening_adjust(world, modes)
