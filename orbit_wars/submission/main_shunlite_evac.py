...
EVAC_THREAT_RATIO_FIXED = 2.0   # enemy must have 2× our garrison to trigger
EVAC_MIN_SHIPS_FIXED = 20       # don't evacuate near-empty planets


class DecisionLogicEvac(DecisionLogic):
    """Shunlite variant: conservative evacuation — 2× threat, 20-ship minimum, no duplicate reinforcement."""
    ...
    def _build_evacuate_missions(self, missions: List[MissionOption]) -> None:
        for planet in self.state.my_planets:
            if self.expired():
                return
            effective = self._effective_planet(planet)
            # Only evacuate planets with a meaningful garrison
            if effective.ships < EVAC_MIN_SHIPS_FIXED:
                continue
            # Check incoming enemy ships from the arrival ledger
            arrivals = self.world.arrivals_by_planet.get(planet.id, [])
            enemy_incoming = sum(
                ships for eta, owner, ships in arrivals
                if owner not in (-1, self.state.player) and ships > 0
            )
            # Only evacuate if enemy overwhelms us 2:1
            if enemy_incoming <= effective.ships * EVAC_THREAT_RATIO_FIXED:
                continue
            # Skip if friendly reinforcements are already en route
            friendly_incoming = any(
                owner == self.state.player for eta, owner, ships in arrivals
            )
            if friendly_incoming:
                continue
            # Check simulation confirms we cannot hold
            hold = self.world.hold_status(planet.id, DEFENSE_HORIZON)
            if hold["holds_full"]:
                continue
            ...
            send = effective.ships
            friendly = sorted(
                [p for p in self.state.my_planets if p.id != planet.id],
                key=lambda p: distance_planets(p, planet),
            )
            for dest in friendly:
                if self.expired():
                ...
                neutrals = sorted(
                    list(self.state.neutral_planets),
                    key=lambda p: distance_planets(p, planet),
                )
                for dest in neutrals[:3]:
                    if self.expired():
...
def agent(obs: Any, config: Any) -> List[List[float | int]]:
    """Evac variant: conservative evacuation with 2× threat threshold."""
    try:
        logic = DecisionLogicEvac(obs, config)
        return logic.decide()
