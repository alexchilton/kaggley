...
SHUNPE_PROD_BONUS_PER_UNIT = 4.0    # additive ships per unit of production
SHUNPE_ANCHOR_BONUS = 10.0          # extra bonus for top-3 highest-prod neutrals
SHUNPE_PHASE_TURNS = 100            # bonus fully tapers off by turn 100
_SHUNPE_ANCHOR_PLANETS: Optional[set] = None


class DecisionLogicProdExpand(DecisionLogic):
    """Shunlite variant: additive production bonus on expand missions, no multiplicative distortion."""

    def _build_all_missions(self) -> List[MissionOption]:
        global _SHUNPE_ANCHOR_PLANETS
        # Reset anchor planets at game start so cross-game state is clean
        if self.state.step == 0:
            _SHUNPE_ANCHOR_PLANETS = None
        if _SHUNPE_ANCHOR_PLANETS is None and self.state.neutral_planets:
            top3 = sorted(self.state.neutral_planets, key=lambda p: -p.production)[:3]
            _SHUNPE_ANCHOR_PLANETS = {p.id for p in top3}

        missions = super()._build_all_missions()
        phase_factor = max(0.0, 1.0 - self.state.step / SHUNPE_PHASE_TURNS)
        ...
        for m in missions:
            ...
            if target is None or target.owner != -1:
                continue
            # Additive production bonus (tapers to zero by turn 100)
            m.score += SHUNPE_PROD_BONUS_PER_UNIT * target.production * phase_factor
            # Permanent anchor bonus for top-3 highest-prod neutrals
            if _SHUNPE_ANCHOR_PLANETS and m.target_id in _SHUNPE_ANCHOR_PLANETS:
                m.score += SHUNPE_ANCHOR_BONUS

        return missions


def agent(obs: Any, config: Any) -> List[List[float | int]]:
    """ProdExpand variant: additive production bonus on expand missions."""
    try:
        logic = DecisionLogicProdExpand(obs, config)
        return logic.decide()
