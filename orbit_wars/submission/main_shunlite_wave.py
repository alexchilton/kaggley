...
class DecisionLogicWave(DecisionLogic):
    """Shunlite variant: boosts targets that 3+ of my planets independently top-score."""

    def _build_all_missions(self) -> List[MissionOption]:
        missions = super()._build_all_missions()
        # For each source planet, find its best-scored non-utility mission target
        best_per_source: Dict[int, Tuple[int, float]] = {}
        for m in missions:
            if m.mission in ("defend", "reinforce", "evacuate"):
                continue
            for sid in m.source_ids:
                prev = best_per_source.get(sid)
                if prev is None or m.score > prev[1]:
                    best_per_source[sid] = (m.target_id, m.score)
        # Count votes per target
        target_votes: Dict[int, int] = {}
        for _sid, (tid, _sc) in best_per_source.items():
            target_votes[tid] = target_votes.get(tid, 0) + 1
        wave_targets = {tid for tid, cnt in target_votes.items() if cnt >= _WAVE_MIN_PLANETS}
        for m in missions:
            if m.target_id in wave_targets and m.mission not in ("defend", "reinforce", "evacuate"):
                m.score += _WAVE_BONUS
        return missions


def agent(obs: Any, config: Any) -> List[List[float | int]]:
    """Wave variant: boosts consensus targets chosen by 3+ of my planets."""
    try:
        logic = DecisionLogicWave(obs, config)
        return logic.decide()
