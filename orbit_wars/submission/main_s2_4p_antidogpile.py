...
class DecisionLogic:
    ...
    def _frontline_reserve(self, planet: Planet) -> int:
        # Anti-dogpile: keep 28% reserve at frontline in crowded 4p early game
        if self._is_4p_crowded() and getattr(self.state, "step", 0) < 60:
            nearest_enemy = min(
                (distance_planets(planet, e) for e in self.state.enemy_planets),
                default=999.0
            )
            if nearest_enemy <= FOUR_PLAYER_FRONTLINE_DISTANCE:
                base_reserve = int(planet.ships * 0.28)
                return max(base_reserve, self.__orig_frontline_reserve(planet))
        return self.__orig_frontline_reserve(planet)

    def __orig_frontline_reserve(self, planet: Planet) -> int:
    ...
    def _is_4p_crowded(self) -> bool:
        """Detect crowded 4p maps: many planets near our home."""
        if getattr(self.state, "num_players", 2) <= 2:
            return False
        if not self.state.my_planets:
            return False
        home = self.state.my_planets[0]
        nearby = sum(1 for p in self.state.all_planets if distance_planets(home, p) <= 22.0)
        return nearby >= 8
