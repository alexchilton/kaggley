...
class DecisionLogic:
    ...
    def _is_long_tight_2p(self) -> bool:
        """Detect 2p maps where all good neutrals are far away."""
        if getattr(self.state, "num_players", 2) > 2:
            return False
        if not self.state.is_opening:
            return False
        neutrals = [p for p in self.state.neutral_planets if p.production >= 2]
        if not neutrals or not self.state.my_planets:
            return False
        home = self.state.my_planets[0]
        best_dist = min(distance_planets(home, n) for n in neutrals)
        return best_dist > 20.0

    def _opening_capture_allowed(self, target: Planet, plan: CapturePlan, mission: str) -> bool:
        if not self.state.is_opening or mission in {"reinforce", "defend", "evacuate"}:
            return True
        if self._is_long_tight_2p():
            eta_limit = 24 if not is_orbiting_planet(target) else 16
        else:
    ...
    def _mission_can_commit(
        ...
        for i, sid in enumerate(mission.source_ids):
            ...
            if self._effective_planet(src).ships < mission.ships[i]:
                return False
            if mission.mission not in ("evacuate",):
                remaining_after = self._effective_planet(src).ships - mission.ships[i]
                hold = self.world.hold_status(sid, min(12, mission.etas[i] + 4),
                                               planned_commitments=self.planned_commitments)
                keep = max(int(hold.get("keep_needed", 0)), self._proactive_keep(self._effective_planet(src)))
                if remaining_after < keep and not self.modes.get("is_finishing"):
                    return False
        return True
