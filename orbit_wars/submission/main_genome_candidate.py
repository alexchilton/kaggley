...
DEFENSE_HORIZON = int(os.environ.get('GEN_DEFENSE_HORIZON', '28'))
COMET_LOOKAHEAD = 28

# ── GENOME WINNER: SMASH+CL28+CURG (60% vs shunlite, Gen 2) ─────────────
GENOME_SUN_ROUTING       = False
GENOME_SMASH_TARGETS     = True
GENOME_COMET_URGENCY     = True
GENOME_INNER_ORBIT       = False
GENOME_INNER_ORBIT_AMT   = 1.08
GENOME_INNER_ORBIT_SHIPS = 40
GEN_SKIP_VALIDATE       = os.environ.get('GEN_SKIP_VALIDATE', '0') == '1'
GEN_PHASE_COMMIT        = os.environ.get('GEN_PHASE_COMMIT', '0') == '1'
GEN_PHASE_COMMIT_MIN_STEP = int(os.environ.get('GEN_PHASE_COMMIT_MIN_STEP', '24'))
GEN_PHASE_COMMIT_TOP_K    = int(os.environ.get('GEN_PHASE_COMMIT_TOP_K', '4'))
GEN_PHASE_COMMIT_ETA_CAP  = int(os.environ.get('GEN_PHASE_COMMIT_ETA_CAP', '40'))
GEN_4P_WIN_MODE          = os.environ.get('GEN_4P_WIN_MODE', '0') == '1'
GEN_4P_SIMPLE_NEED       = os.environ.get('GEN_4P_SIMPLE_NEED', '0') == '1'
GEN_4P_FORCE_RATIO       = float(os.environ.get('GEN_4P_FORCE_RATIO', '0.45'))
GEN_4P_RESERVE_SCALE     = float(os.environ.get('GEN_4P_RESERVE_SCALE', '0.35'))
GEN_4P_PHASE_SCORE_BONUS = float(os.environ.get('GEN_4P_PHASE_SCORE_BONUS', '1.55'))
# ─────────────────────────────────────────────────────────────────────────────
...
ATTACK_COST_WEIGHT = float(os.environ.get('GEN_ATTACK_COST_WEIGHT', '0.75'))
SNIPE_COST_WEIGHT = float(os.environ.get('GEN_SNIPE_COST_WEIGHT', '1.08'))
...
STATIC_SCORE_MULT = float(os.environ.get('GEN_STATIC_SCORE_MULT', '1.18'))
EARLY_STATIC_SCORE_MULT = float(os.environ.get('GEN_EARLY_STATIC_SCORE_MULT', '1.25'))
SNIPE_SCORE_MULT = float(os.environ.get('GEN_SNIPE_SCORE_MULT', '1.12'))
SWARM_SCORE_MULT = float(os.environ.get('GEN_SWARM_SCORE_MULT', '1.06'))
CRASH_EXPLOIT_SCORE_MULT = 1.05

NEUTRAL_MARGIN_BASE = int(os.environ.get('GEN_NEUTRAL_MARGIN_BASE', '2'))
HOSTILE_MARGIN_BASE = int(os.environ.get('GEN_HOSTILE_MARGIN_BASE', '2'))
MARGIN_PROD_WEIGHT = int(os.environ.get('GEN_MARGIN_PROD_WEIGHT', '2'))
NEUTRAL_MARGIN_CAP = int(os.environ.get('GEN_NEUTRAL_MARGIN_CAP', '8'))
HOSTILE_MARGIN_CAP = int(os.environ.get('GEN_HOSTILE_MARGIN_CAP', '8'))
STATIC_MARGIN = int(os.environ.get('GEN_STATIC_MARGIN', '4'))
CONTESTED_MARGIN = 5
...
PROACTIVE_KEEP_RATIO = float(os.environ.get('GEN_PROACTIVE_KEEP_RATIO', '0.18'))
FORCE_CONCENTRATION_MIN_RATIO = 0.65
STACKED_PROACTIVE_KEEP_RATIO = float(os.environ.get('GEN_STACKED_PROACTIVE_KEEP_RATIO', '0.22'))
FOUR_PLAYER_CAUTIOUS_LAUNCH_CAP = int(os.environ.get('GEN_4P_CAUTIOUS_LAUNCH_CAP', '6'))
FOUR_PLAYER_PRESSURED_LAUNCH_CAP = int(os.environ.get('GEN_4P_PRESSURED_LAUNCH_CAP', '3'))
FOUR_PLAYER_RECOVERY_LAUNCH_CAP = int(os.environ.get('GEN_4P_RECOVERY_LAUNCH_CAP', '5'))
FOUR_PLAYER_FRONTLINE_DISTANCE = 24.0
FOUR_PLAYER_FRONTLINE_RESERVE_RATIO = float(os.environ.get('GEN_4P_FRONTLINE_RESERVE', '0.0'))
FOUR_PLAYER_DOUBLE_FRONT_RESERVE_RATIO = float(os.environ.get('GEN_4P_DOUBLE_FRONT_RESERVE', '0.0'))
FOUR_PLAYER_HOSTILE_VALUE_DAMP = float(os.environ.get('GEN_4P_HOSTILE_DAMP', '0.88'))
FOUR_PLAYER_SAFE_NEUTRAL_BONUS = float(os.environ.get('GEN_4P_NEUTRAL_BONUS', '1.06'))
FOUR_PLAYER_PIVOT_PROD_RATIO = 1.18
FOUR_PLAYER_PIVOT_STAGE_DISTANCE = 28.0
FOUR_PLAYER_PIVOT_SAFE_NEUTRAL_LIMIT = 2
FOUR_PLAYER_PIVOT_HOSTILE_BONUS = float(os.environ.get('GEN_4P_PIVOT_HOSTILE_BONUS', '1.18'))
FOUR_PLAYER_PREP_HOSTILE_DAMP = float(os.environ.get('GEN_4P_PREP_HOSTILE_DAMP', '0.92'))
FOUR_PLAYER_RUNAWAY_STRENGTH_RATIO = 1.18
FOUR_PLAYER_RUNAWAY_PROD_RATIO = 1.12
FOUR_PLAYER_RUNAWAY_PRIORITY = float(os.environ.get('GEN_4P_RUNAWAY_PRIORITY', '1.34'))
FOUR_PLAYER_RUNAWAY_HOSTILE_BONUS = float(os.environ.get('GEN_4P_RUNAWAY_HOSTILE_BONUS', '1.18'))
FOUR_PLAYER_RECOVERY_HOSTILE_BONUS = float(os.environ.get('GEN_4P_RECOVERY_HOSTILE_BONUS', '1.10'))
FOUR_PLAYER_RECOVERY_NEUTRAL_DAMP = float(os.environ.get('GEN_4P_RECOVERY_NEUTRAL_DAMP', '0.90'))
...
def _sun_safe_angle(
    ...
    """Compute a tangent angle that clears the sun, biased toward the target. Returns None if inside sun."""
    direct = math.atan2(end_y - start_y, end_x - start_x)
    if not segment_circle_intersects(
        (start_x, start_y), (end_x, end_y), SUN_CENTER, SUN_RADIUS + margin
    ):
        return direct
    d = math.hypot(start_x - CENTER, start_y - CENTER)
    if d <= SUN_RADIUS + 1.0:
        return None
    half = math.asin(min(1.0, (SUN_RADIUS + 1.0) / d))
    to_sun = math.atan2(CENTER - start_y, CENTER - start_x)
    cw = to_sun + half
    ccw = to_sun - half
    def adiff(a: float) -> float:
        dd = (a - direct) % (2.0 * math.pi)
        return dd if dd < math.pi else 2.0 * math.pi - dd
...
class InterceptSolver:
    ...
    def _estimate_to_position(
        ...
        Returns (angle, eta, sun_routed) or None.
        """
        ...
        sun_routed = GENOME_SUN_ROUTING and segment_circle_intersects(
            (from_x, from_y), (to_x, to_y), SUN_CENTER, SUN_RADIUS,
        )
        return normalize_angle(angle), eta, sun_routed
    ...
    def _search_safe_intercept(
        ...
        for candidate_turn in range(1, max_turns + 1):
            ...
            if estimate is None:
                continue
            angle, eta, sun_routed = estimate
            delta = abs(eta - candidate_turn)
            if delta > 1:
                continue
            if sun_routed or GEN_SKIP_VALIDATE:
                hit_turn = eta
            else:
    ...
    def solve_intercept(
        ...
        try:
            ...
            angle, eta, sun_routed = fixed

            # Sun-routed tangent shots: trust the angle and ETA, skip iterative refinement
            # and validation (the fleet won't hit the exact target position in simulation
            # but the tangent path clears the sun and heads toward the target hemisphere)
            if sun_routed:
                return angle, eta
            ...
            for _ in range(5):
                ...
                if next_estimate is None:
                    return self._search_safe_intercept(from_x, from_y, from_radius, target_planet, num_ships)
                next_angle, next_eta, next_sun_routed = next_estimate
                if next_sun_routed:
...
class DecisionLogic:
    ...
    def _proactive_keep(self, planet: Planet) -> int:
        if self.state.num_players < 4 or not self.state.enemy_planets:
            return 0
        reserve_scale = GEN_4P_RESERVE_SCALE if self._structural_four_player_active() else 1.0
        proactive = 0
        ...
        for enemy in nearby_enemies:
            ...
            if eta <= PROACTIVE_DEFENSE_HORIZON:
                proactive = max(proactive, int(enemy.ships * PROACTIVE_KEEP_RATIO * reserve_scale))
        ...
        return max(proactive, int(best_stacked * STACKED_PROACTIVE_KEEP_RATIO * reserve_scale))
    ...
    def _frontline_reserve(self, planet: Planet) -> int:
        ...
        reserve_scale = GEN_4P_RESERVE_SCALE if self._structural_four_player_active() else 1.0
        if nearby >= 2:
            return int(planet.ships * FOUR_PLAYER_DOUBLE_FRONT_RESERVE_RATIO * reserve_scale)
        if nearby == 1 and not self.modes.get("is_ahead", False):
            return int(planet.ships * FOUR_PLAYER_FRONTLINE_RESERVE_RATIO * reserve_scale)
        return 0
    ...
    def _structural_four_player_active(self) -> bool:
        return GEN_4P_WIN_MODE and GEN_PHASE_COMMIT and self.state.num_players >= 4

    def _phase_commit_budget(self, planet: Planet) -> int:
        if not self._structural_four_player_active():
            return self._planet_surplus(planet)
        effective = self._effective_planet(planet)
        if effective.owner != self.state.player:
            return 0
        hard_keep = max(4, int(effective.ships * 0.08))
        return max(self._planet_surplus(planet), max(0, int(effective.ships) - hard_keep))

    def _simple_phase_need(self, target: Planet, eta: int) -> int:
        if not GEN_4P_SIMPLE_NEED or target.owner in (-1, self.state.player):
            return max(1, int(target.ships) + 1)
        base = float(target.ships) + float(target.production) * max(1, eta)
        return max(1, int(math.ceil(base * 1.05 + 2.0)))
    ...
    def _preferred_send(self, target: Planet, need: int, eta: int, available: int, mission: str) -> int:
        if GENOME_SMASH_TARGETS and mission in {'attack', 'swarm', 'snipe'}:
            if target.owner not in (-1, self.state.player):
                e_x, e_y = target.x, target.y
                nearby_my = sum(
                    p.ships for p in self.state.my_planets
                    if (p.x - e_x) ** 2 + (p.y - e_y) ** 2 < 2500
                )
                if nearby_my > target.ships * 1.25 and nearby_my > 0:
    ...
    def _build_modes(self) -> Dict[str, Any]:
        ...
        leader_owner = standings[0][0] if standings else self.state.player
        ...
        second_prod = max(
            (owner_prod.get(owner, 0) for owner, _ in standings if owner != leader_owner),
            default=0,
        )
        ...
        runaway_owner: Optional[int] = None
        if self.state.num_players >= 4:
            ...
            if (
                leader_owner != self.state.player
                and leader_strength > max(1, second_strength) * FOUR_PLAYER_RUNAWAY_STRENGTH_RATIO
                and owner_prod.get(leader_owner, 0) > max(1, second_prod) * FOUR_PLAYER_RUNAWAY_PROD_RATIO
            ):
                runaway_owner = leader_owner
            four_player_recovery_mode = (
                not is_ahead
                and not is_dominating
                and (
                    is_behind
                    or (
                        runaway_owner is not None
                        and my_rank >= 2
                        and self.state.step >= 24
                    )
                    or (
                        my_rank >= 3
                        and my_prod <= max(2, max_enemy_prod // 2)
                        and self.state.step >= 24
                    )
                )
            )
            four_player_pivot_ready = (
                ...
                and (
                    ...
                    or (
                        four_player_recovery_mode
                        and runaway_owner is not None
                        and self.state.step >= 40
                    )
                )
            )
        ...
        return {
            ...
            "four_player_recovery_mode": four_player_recovery_mode,
            "runaway_owner": runaway_owner,
    ...
    def _build_enemy_priority(self) -> Dict[int, float]:
        ...
        runaway_owner = self.modes.get("runaway_owner")
        leader_owner = self.modes.get("leader_owner")
        recovery_mode = bool(self.modes.get("four_player_recovery_mode"))
        for idx, (owner, _score) in enumerate(ranked):
            weight = max(0.82, 1.18 - 0.12 * idx)
            if owner == leader_owner and self.state.num_players >= 4 and self.modes.get("my_rank", 1) >= 2:
                weight = max(weight, 1.22)
            if owner == runaway_owner:
                weight = max(weight, FOUR_PLAYER_RUNAWAY_PRIORITY if recovery_mode else 1.26)
            elif runaway_owner is not None and recovery_mode:
                weight *= 0.92
            priority[owner] = weight
        return priority
    ...
    def _turn_launch_cap(self) -> int:
        ...
        if (
            num_players >= 4
            and self._structural_four_player_active()
            and getattr(self.state, "step", 0) >= GEN_PHASE_COMMIT_MIN_STEP
        ):
            return MAX_TURN_LAUNCHES
        if num_players >= 4 and not modes.get("is_cleanup"):
            if modes.get("four_player_recovery_mode"):
                return min(MAX_TURN_LAUNCHES, FOUR_PLAYER_RECOVERY_LAUNCH_CAP)
    ...
    def _target_value(self, target: Planet, eta: int, mission: str) -> float:
        ...
        if modes["is_behind"] and target.owner == -1 and self._is_safe_neutral(target):
            value *= 1.08
        if self.state.num_players <= 2 and target.owner == -1 and self._is_safe_neutral(target):
            if target.production >= 3 and modes.get("my_prod", 0) < modes.get("max_enemy_prod", 0):
                value *= 1.08
        if (
            self.state.num_players <= 2
            and target.owner not in (-1, self.state.player)
            and modes.get("my_prod", 0) + 1 < modes.get("max_enemy_prod", 0)
            and not modes.get("is_finishing")
        ):
            value *= 0.95
        ...
        if self.state.num_players >= 4 and target.owner not in (-1, self.state.player):
            ...
            if target.owner == modes.get("runaway_owner"):
                value *= FOUR_PLAYER_RUNAWAY_HOSTILE_BONUS
            elif modes.get("four_player_recovery_mode") and modes.get("runaway_owner") is not None:
                value *= 0.94
            if modes.get("four_player_pivot_ready"):
                value *= FOUR_PLAYER_PIVOT_HOSTILE_BONUS
            elif modes.get("four_player_recovery_mode"):
                value *= FOUR_PLAYER_RECOVERY_HOSTILE_BONUS
        ...
        if (
            ...
            if modes.get("four_player_map_saturated"):
                value *= 0.96
            if modes.get("four_player_recovery_mode") and modes.get("four_player_safe_neutral_count", 0) > FOUR_PLAYER_PIVOT_SAFE_NEUTRAL_LIMIT:
                value *= FOUR_PLAYER_RECOVERY_NEUTRAL_DAMP

        # GENOME: smash_targets — decisive local superiority bonus
        if GENOME_SMASH_TARGETS and mission in {'attack', 'swarm', 'snipe'}:
            if target.owner not in (-1, self.state.player):
                e_x, e_y = target.x, target.y
                nearby_my_ships = sum(
                ...
                if nearby_my_ships > target.ships * 1.25 and nearby_my_ships > 0:
                    value *= 4.0

        # GENOME: conditional inner orbit bonus
        if GENOME_INNER_ORBIT and mission in {'expand', 'attack', 'snipe'}:
            orbit_r = math.hypot(target.x - CENTER, target.y - CENTER)
            if (orbit_r + target.radius) < 46.0 and target.ships < GENOME_INNER_ORBIT_SHIPS:
                if not self.state.is_early and not self.state.is_opening:
                    value *= GENOME_INNER_ORBIT_AMT
    ...
    def _phase_commit_profile(self) -> Optional[Dict[str, Any]]:
        if (
            not GEN_PHASE_COMMIT
            or self.state.num_players < 4
            or self.state.step < GEN_PHASE_COMMIT_MIN_STEP
            or not self.state.enemy_planets
        ):
            return None

        modes = self.modes
        aggressive = self._structural_four_player_active()
        safe_neutral_count = int(modes.get("four_player_safe_neutral_count", 0))
        if (
            not aggressive
            and self.state.is_opening
            and safe_neutral_count >= max(3, FOUR_PLAYER_PIVOT_SAFE_NEUTRAL_LIMIT + 2)
        ):
            return None

        runaway_owner = modes.get("runaway_owner")
        if modes.get("is_finishing"):
            return {"phase": "finish", "ratio": 0.92 if aggressive else 0.84, "bonus": 1.55 if aggressive else 1.30, "preferred_owner": None, "max_eta": min(GEN_PHASE_COMMIT_ETA_CAP, 42 if aggressive else 40)}
        if modes.get("four_player_recovery_mode") and runaway_owner is not None:
            return {"phase": "dogpile", "ratio": 0.88 if aggressive else 0.78, "bonus": 1.50 if aggressive else 1.28, "preferred_owner": runaway_owner, "max_eta": min(GEN_PHASE_COMMIT_ETA_CAP, 40 if aggressive else 38)}
        if modes.get("is_dominating"):
            return {"phase": "crush", "ratio": 0.90 if aggressive else 0.80, "bonus": 1.48 if aggressive else 1.22, "preferred_owner": None, "max_eta": min(GEN_PHASE_COMMIT_ETA_CAP, 38 if aggressive else 36)}
        if modes.get("four_player_pivot_ready") or modes.get("is_ahead"):
            preferred_owner = runaway_owner if runaway_owner is not None else modes.get("leader_owner")
            return {"phase": "pivot", "ratio": 0.82 if aggressive else 0.68, "bonus": 1.38 if aggressive else 1.15, "preferred_owner": preferred_owner, "max_eta": min(GEN_PHASE_COMMIT_ETA_CAP, 36 if aggressive else 34)}
        if aggressive and modes.get("my_rank", 1) >= 2 and self.state.step >= max(18, GEN_PHASE_COMMIT_MIN_STEP - 6):
            return {"phase": "contest", "ratio": 0.78, "bonus": 1.30, "preferred_owner": modes.get("leader_owner"), "max_eta": min(GEN_PHASE_COMMIT_ETA_CAP, 36)}
        if modes.get("my_rank", 1) >= 3:
            return {"phase": "recovery", "ratio": 0.78 if aggressive else 0.70, "bonus": 1.26 if aggressive else 1.12, "preferred_owner": modes.get("leader_owner"), "max_eta": min(GEN_PHASE_COMMIT_ETA_CAP, 36 if aggressive else 34)}
        return None

    def _build_phase_commit_missions(self, missions: List[MissionOption]) -> None:
        profile = self._phase_commit_profile()
        if profile is None:
            return
        ...
        preferred_owner = profile.get("preferred_owner")
        target_scores: List[Tuple[float, Planet]] = []
        for target in self.state.enemy_planets:
            sample_shot: Optional[Tuple[int, int]] = None
            for donor in sorted(available, key=lambda src: distance_planets(src, target))[:3]:
                budget = self._phase_commit_budget(donor)
                if budget <= 0:
                    continue
                send = min(budget, max(6, int(budget * profile["ratio"])))
                if send < 6:
                    continue
                shot = self._plan_shot(donor, target, send)
                if shot is None:
                    continue
                sample_shot = (send, shot[1])
                break
            if sample_shot is None:
                continue
            _send, eta = sample_shot
            if eta > profile["max_eta"]:
                continue
            value = self._target_value(target, eta, "attack")
            if value <= 0:
                continue
            score = value / max(1.0, float(eta))
            score *= self.enemy_priority.get(target.owner, 1.0)
            if preferred_owner is not None and target.owner == preferred_owner:
                score *= 1.18
            target_scores.append((score, target))

        target_scores.sort(key=lambda item: -item[0])
        launch_limit = max(2, min(self._turn_launch_cap(), GEN_PHASE_COMMIT_TOP_K + (1 if aggressive else 0)))
        built = 0

        for _score, target in target_scores[:4]:
            donor_plans: List[Tuple[Planet, int, float, int]] = []
            donors = sorted(
                available,
                key=lambda src: (distance_planets(src, target), -self._phase_commit_budget(src)),
            )
            for donor in donors:
                budget = self._phase_commit_budget(donor)
                if budget <= 0:
                ...
                if send < 6:
                    continue
                shot = self._plan_shot(donor, target, send)
                if shot is None or shot[1] > profile["max_eta"]:
                    continue
                donor_plans.append((donor, send, shot[0], shot[1]))
                if len(donor_plans) >= launch_limit:
                    break

            if len(donor_plans) < 2:
                continue

            donor_plans.sort(key=lambda item: (item[3], -item[1]))
            chosen: List[Tuple[Planet, int, float, int]] = []
            need = 1
            max_eta = 0
            for donor_plan in donor_plans:
                chosen.append(donor_plan)
                max_eta = max(entry[3] for entry in chosen)
                if aggressive and GEN_4P_SIMPLE_NEED:
                    need = self._simple_phase_need(target, max_eta)
                else:
                    extra = tuple((entry[3], self.state.player, entry[1]) for entry in chosen)
                ...
                if need <= 0 and len(chosen) >= 2:
                    break
                if aggressive and sum(entry[1] for entry in chosen) >= need and len(chosen) >= 2:
                    break

            total_ships = sum(entry[1] for entry in chosen)
            if aggressive and total_ships < need:
                continue
            if need > 0 and not aggressive:
                continue
            if len(chosen) < 2:
                continue

            value = self._target_value(target, max_eta, "attack")
            if value <= 0:
                continue
            score = self._score_mission(value, total_ships, max_eta, target, "swarm")
            score *= profile["bonus"] * (GEN_4P_PHASE_SCORE_BONUS if aggressive else 1.0)
            if preferred_owner is not None and target.owner == preferred_owner:
                score *= 1.12 if aggressive else 1.08
            missions.append(MissionOption(
                score=score,
                source_ids=[entry[0].id for entry in chosen],
                target_id=target.id,
                angles=[entry[2] for entry in chosen],
                etas=[entry[3] for entry in chosen],
                ships=[entry[1] for entry in chosen],
                needed=max(need, total_ships if aggressive else 0),
                mission="swarm",
            ))
            built += 1
            if not aggressive or built >= 2:
                return
    ...
    def _build_all_missions(self) -> List[MissionOption]:
        ...
        self._build_phase_commit_missions(missions)
    ...
    def _build_comet_missions(self, missions: List[MissionOption]) -> None:
        ...
        for target in sorted(comet_targets, key=lambda t: (-t.production, t.ships)):
            ...
            # GENOME: comet urgency boost near spawn windows
            if GENOME_COMET_URGENCY:
                _COMET_SPAWN_TURNS = frozenset({50, 150, 250, 350, 450})
                _dist_to_next = min(
                    (sp - self.state.step) % 100
                    for sp in _COMET_SPAWN_TURNS
                )
                if _dist_to_next <= 10:
                    score *= 1.08
            missions.append(MissionOption(
    ...
    def _build_attack_missions(self, missions: List[MissionOption]) -> None:
        ...
        if self._shun_opening_active():
            return
        aggressive_4p = self._structural_four_player_active()
        recovery_mode = self.state.num_players >= 4 and self.modes.get("four_player_recovery_mode")
        if (
            self.state.num_players >= 4
            and not aggressive_4p
            and not self.modes.get("four_player_pivot_ready")
        ...
        if (
            ...
            if len(safe_neutrals) >= 2:
                return
        if self.state.is_early and not recovery_mode and not aggressive_4p:
            good_neutrals = [p for p in self.state.neutral_planets if p.production >= OPENING_MIN_PRODUCTION]
            if len(good_neutrals) >= 3:
                return
        elif self.state.is_opening and len(self.state.neutral_planets) > 6 and not recovery_mode and not aggressive_4p:
            return
        if self.state.num_players <= 2:
            targets = sorted(
                self.state.enemy_planets,
    ...
    def _build_swarm_missions(self, missions: List[MissionOption]) -> None:
        if (self.state.is_opening and not self._structural_four_player_active()) or not self.state.enemy_planets:
            return
        available = self._available_my_planets()
        if len(available) < 2:
            return
    ...
    def _mission_can_commit(
        ...
        if self._mission_blocks_target(mission) and (
            ...
            return False
        # Force concentration: skip attacks sending < 65% of needed ships
        if mission.mission in ("attack", "snipe", "swarm") and mission.needed > 0:
            total_ships = sum(mission.ships)
            force_ratio = GEN_4P_FORCE_RATIO if self._structural_four_player_active() else FORCE_CONCENTRATION_MIN_RATIO
            if total_ships < mission.needed * force_ratio:
                return False
    ...
    # ---------------------------------------------------------------
    # RL policy helpers (linear softmax reranker)
    # ---------------------------------------------------------------

    _RL_POLICY_WEIGHTS_2P = [
        -1.1826706261037206e-17, 7.975606635418543e-16, 5.210204941709292e-16, 1.9034765745103694e-16,
        8.40714172257741e-17, 1.712339625451803e-16, 7.155518823943213e-17, 0.0,
        -6.555405476293437e-17, -7.503980781341526e-17, 1.5580945129681014e-18, 4.150670897618205,
        1.1730349757682543, -0.7851347755889183, -1.4610960626448775, -0.12600123960313264,
        -1.488867448297972, 0.8188769175313396, 0.5073727202478256, 0.4378020006417264,
        0.2959616566918502, -0.7337636573335781, 0.8060992563464094, 0.5309627746648581,
        -0.34640332285699105, 0.39949708433713044, -0.5017664916378632, -0.017819798347089646,
        -0.09264498554603476, -0.7779245169604165, 0.009925474501700665, -2.0336978920127712e-17,
        0.0,
    ]
    _RL_POLICY_WEIGHTS_4P = [
        -1.1826706261037206e-17, 7.975606635418543e-16, 5.210204941709292e-16, 1.9034765745103694e-16,
        8.40714172257741e-17, 1.712339625451803e-16, 7.155518823943213e-17, 0.0,
        -6.555405476293437e-17, -7.503980781341526e-17, 1.5580945129681014e-18, 4.150670897618205,
        1.1730349757682543, -0.7851347755889183, -1.4610960626448775, -0.12600123960313264,
    ...
    _RL_POLICY_TEMPERATURE = 1.0

    def _rl_clip01(self, value: float) -> float:
        return max(0.0, min(1.0, value))

    def _rl_safe_ratio(self, num: float, denom: float, default: float = 0.0) -> float:
        return num / denom if denom > 0 else default

    def _rl_tanh_scale(self, value: float, scale: float) -> float:
        if scale <= 0:
            return 0.0
        return math.tanh(value / scale)

    def _rl_build_state_snapshot(self) -> Dict[str, float]:
        state = self.state
        modes = self.modes
        ...
        return {
            "step_frac": self._rl_clip01(float(state.step) / 200.0),
            "remaining_frac": self._rl_clip01(float(state.remaining_steps) / 200.0),
            ...
            "my_ship_share": self._rl_clip01(self._rl_safe_ratio(my_total, my_total + enemy_total, default=0.5)),
            "my_prod_share": self._rl_clip01(self._rl_safe_ratio(my_prod, my_prod + max_enemy_prod, default=0.5)),
            "lead_ratio": self._rl_clip01(self._rl_safe_ratio(my_total, lead_base, default=1.0)),
        }

    def _rl_build_feature_vector(self, mission: Any, base_value: float, existing_moves: int, turn_launch_cap: int) -> List[float]:
        state = self.state
        snapshot = self._rl_build_state_snapshot()
        ...
        return [
            ...
            snapshot["lead_ratio"],
            self._rl_tanh_scale(float(mission.score), 80.0),
            self._rl_tanh_scale(float(base_value), 90.0),
            self._rl_clip01(self._rl_safe_ratio(float(max(mission.etas, default=0)), 28.0)),
            self._rl_clip01(self._rl_safe_ratio(total_send, 80.0)),
            self._rl_clip01(self._rl_safe_ratio(float(len(mission.source_ids)), 4.0)),
            self._rl_clip01(self._rl_safe_ratio(float(mission.needed), 80.0)),
            self._rl_clip01(self._rl_safe_ratio(target_prod, 10.0)),
            self._rl_clip01(self._rl_safe_ratio(target_ships, 80.0)),
            ...
            float(self.enemy_priority.get(target_owner, 1.0)),
            self._rl_clip01(self._rl_safe_ratio(float(existing_moves), float(max(1, turn_launch_cap)))),
            0.0,  # is_defer
        ]

    def _rl_score_candidates(self, feature_vectors: List[List[float]]) -> int:
        """Score candidates with split linear policy, return argmax index."""
        is_2p = self.state.num_players <= 2
        weights = self._RL_POLICY_WEIGHTS_2P if is_2p else self._RL_POLICY_WEIGHTS_4P
        best_idx = 0
        best_score = -float("inf")
        for i, fv in enumerate(feature_vectors):
            score = 0.0
            for j in range(min(len(weights), len(fv))):
                score += weights[j] * fv[j]
            score /= self._RL_POLICY_TEMPERATURE
            if score > best_score:
                best_score = score
                best_idx = i
        return best_idx

    def _rl_window_open(self) -> bool:
        """Check if the RL reranker should activate."""
        step = self.state.step
        if step < 24 or step > 180:
            return False
        remaining = self.state.remaining_steps
        if remaining < VERY_LATE_REMAINING_TURNS:
            return False
        ...
        if modes.get("is_finishing"):
            return False
        if self.state.num_players <= 2:
            ship_share = self._rl_safe_ratio(
                float(modes.get("my_total", 0)),
                float(modes.get("my_total", 0)) + float(modes.get("enemy_total", 0)),
                default=0.5,
            )
            if ship_share < 0.25 or ship_share > 0.80:
                return False
        else:
            my_rank = modes.get("my_rank", 1)
            if my_rank > 3:
                return False
            if modes.get("is_dominating"):
                return False
    ...
    def _commit_missions(self, missions: List[MissionOption]) -> List[PlannedMove]:
        ...
        turn_launch_cap = self._turn_launch_cap()
        rl_active = self._rl_window_open()
        ...
        while remaining and not self.expired() and len(moves) < turn_launch_cap:
            ...
            # RL reranker: attempt to pick a better candidate
            if rl_active and len(remaining) >= 2:
                # Collect committable candidates in horizon
                committable = []
                for idx in range(horizon):
                    m = remaining[idx]
                    if self._mission_can_commit(m, existing_moves=len(moves)):
                        committable.append((idx, m))

                if len(committable) >= 2:
                    feature_vectors: List[List[float]] = []
                    # Index 0 = defer option (heuristic best)
                    heuristic_best_mission = remaining[best_idx]
                    heuristic_best_fv = self._rl_build_feature_vector(
                        heuristic_best_mission, best_value, len(moves), turn_launch_cap
                    )
                    defer_fv = list(heuristic_best_fv)
                    defer_fv[-1] = 1.0  # is_defer
                    feature_vectors.append(defer_fv)

                    candidate_indices = []
                    for idx, m in committable:
                        fv = self._rl_build_feature_vector(m, m.score, len(moves), turn_launch_cap)
                        feature_vectors.append(fv)
                        candidate_indices.append(idx)

                    rl_choice = self._rl_score_candidates(feature_vectors)
                    if rl_choice > 0:
                        # RL picked a real candidate
                        best_idx = candidate_indices[rl_choice - 1]
...
def _adapt_to_map(obs: Any) -> None:
    """Detect map geometry on first call and adjust scoring constants."""
    global _MAP_ADAPTED, ATTACK_COST_WEIGHT, SNIPE_COST_WEIGHT, STATIC_SCORE_MULT
    global EARLY_STATIC_SCORE_MULT
    if _MAP_ADAPTED:
        return
    _MAP_ADAPTED = True

    # Extract planet positions from observation
    planets_raw = obs.get("planets", []) if isinstance(obs, dict) else getattr(obs, "planets", [])
    if not planets_raw or len(planets_raw) < 4:
        return

    # Compute avg pairwise distance as spread metric
    coords = []
    ...
    # Adapt based on genome sweep findings:
    # Tight maps (<59): cheaper attacks work, keep static mult high
    # Spread maps (>61): cheaper snipes work, lower static mult
    if avg_pair_dist < 59.0:
        # Tight map - attacks are cheap, static captures valuable
        ATTACK_COST_WEIGHT = 0.65
        SNIPE_COST_WEIGHT = 1.08
        STATIC_SCORE_MULT = 1.18
        EARLY_STATIC_SCORE_MULT = 1.35
    elif avg_pair_dist > 61.0:
        # Spread map - snipes valuable, static less so
        ATTACK_COST_WEIGHT = 0.90
        SNIPE_COST_WEIGHT = 0.85
        STATIC_SCORE_MULT = 1.0
        EARLY_STATIC_SCORE_MULT = 1.25
    # else: keep defaults (mid-range maps)


def agent(obs: Any, config: Any) -> List[List[float | int]]:
    """Kaggle Orbit Wars entrypoint with RL-enhanced mission selection."""
    try:
        _adapt_to_map(obs)
