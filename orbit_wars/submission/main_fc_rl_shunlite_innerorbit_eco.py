...
INNER_ORBIT_BONUS = 1.10       # stronger bonus, but only fires in mid/late game
INNER_ORBIT_MAX_SHIPS = 60     # slightly more generous ship threshold
...
def is_inner_orbit_planet(planet: Planet) -> bool:
    """Return whether a planet is in the inner orbit zone (closer to sun)."""
    orbital_radius = distance_xy(planet.x, planet.y, CENTER, CENTER)
...
class DecisionLogic:
    ...
    def _target_value(self, target: Planet, eta: int, mission: str) -> float:
        ...
        # Inner orbit bonus: only mid/late game when economy is established
        if (
            is_inner_orbit_planet(target)
            and mission in {"expand", "attack", "snipe"}
            and target.ships < INNER_ORBIT_MAX_SHIPS
            and not self.state.is_early
            and not self.state.is_opening
        ):
            value *= INNER_ORBIT_BONUS
