...
INNER_ORBIT_THRESHOLD = 46.0
INNER_ORBIT_VALUE_BONUS = 1.12
PATH_CLEAR_INNER_ORBIT_BOOST = 1.07   # extra score mult when path to inner-orbit target clears the sun
COMET_SPAWN_TURNS = [50, 150, 250, 350, 450]
COMET_SPAWN_URGENCY_WINDOW = 12       # turns before spawn where comet score gets boosted
COMET_SPAWN_SCORE_BOOST = 1.15        # score multiplier when spawn is imminent
...
FOUR_PLAYER_RUNAWAY_PRIORITY = 1.34
FOUR_PLAYER_RUNAWAY_HOSTILE_BONUS = 1.18
FOUR_PLAYER_RECOVERY_HOSTILE_BONUS = 1.10
FOUR_PLAYER_RECOVERY_NEUTRAL_DAMP = 0.90
FOUR_PLAYER_LONG_NEUTRAL_ETA = 15
FOUR_PLAYER_LONG_ORBITING_ETA = 11
FOUR_PLAYER_TIGHT_RACE_DAMP = 0.84
FOUR_PLAYER_LONG_NEUTRAL_DAMP = 0.86
FOUR_PLAYER_LONG_ORBITING_DAMP = 0.80
FOUR_PLAYER_CROWDED_LONG_DAMP = 0.92
FOUR_PLAYER_LOCAL_STATIC_BONUS = 1.06
FOUR_PLAYER_SAFE_RACE_BONUS = 1.03
...
def is_inner_orbit_planet(planet: Planet) -> bool:
    """Return True for planets in a very tight orbit close to the sun (fast orbital period)."""
    orbital_radius = distance_xy(planet.x, planet.y, CENTER, CENTER)
    return orbital_radius + planet.radius < INNER_ORBIT_THRESHOLD
...
class DecisionLogic:
    ...
    def _is_4p_crowded(self) -> bool:
        if self.state.num_players < 4 or not self.state.my_planets:
            return False
        home = max(self.state.my_planets, key=lambda planet: (planet.production, planet.ships))
        nearby = sum(
            1
            for planet in self.state.planets
            if distance_planets(home, planet) <= FOUR_PLAYER_PIVOT_STAGE_DISTANCE
        )
        return nearby >= 8
    ...
    def _frontline_reserve(self, planet: Planet) -> int:
        ...
        reserve = 0
        if nearby >= 2:
            reserve = int(planet.ships * FOUR_PLAYER_DOUBLE_FRONT_RESERVE_RATIO)
        elif nearby == 1 and not self.modes.get("is_ahead", False):
            reserve = int(planet.ships * FOUR_PLAYER_FRONTLINE_RESERVE_RATIO)
        if (
            nearby >= 1
            and self.modes.get("four_player_crowded")
            and getattr(self.state, "step", 0) < FOUR_PLAYER_CROWDED_TURN_LIMIT
        ):
            reserve = max(reserve, int(planet.ships * FOUR_PLAYER_CROWDED_FRONT_RESERVE_RATIO))
        return reserve
    ...
    def _build_modes(self) -> Dict[str, Any]:
        ...
        four_player_crowded = False
        runaway_owner: Optional[int] = None
        if self.state.num_players >= 4:
            four_player_crowded = self._is_4p_crowded()
        ...
        return {
            ...
            "four_player_crowded": four_player_crowded,
    ...
    def _turn_launch_cap(self) -> int:
        ...
        if num_players >= 4 and not modes.get("is_cleanup"):
            ...
            if (
                modes.get("four_player_crowded")
                and getattr(self.state, "step", 0) < FOUR_PLAYER_CROWDED_TURN_LIMIT
                and not modes.get("four_player_pivot_ready")
            ):
    ...
    def _four_player_neutral_position_multiplier(self, target: Planet, eta: int) -> float:
        if (
            ...
            or target.owner != -1
            or getattr(self.state, "step", 0) >= 100
        ...
        crowded = bool(self.modes.get("four_player_crowded"))
        mult = 1.0

        if race_gap <= 0:
            mult *= 0.78
        elif race_gap == 1:
            mult *= FOUR_PLAYER_TIGHT_RACE_DAMP
        elif race_gap == 2:
            mult *= 0.93

        if eta >= FOUR_PLAYER_LONG_NEUTRAL_ETA:
            mult *= FOUR_PLAYER_LONG_NEUTRAL_DAMP
        elif is_orbiting_planet(target) and eta >= FOUR_PLAYER_LONG_ORBITING_ETA:
            mult *= FOUR_PLAYER_LONG_ORBITING_DAMP

        if crowded and eta >= 12:
            mult *= FOUR_PLAYER_CROWDED_LONG_DAMP

        if not is_orbiting_planet(target) and eta <= 10 and race_gap >= SAFE_NEUTRAL_MARGIN + 1:
            mult *= FOUR_PLAYER_LOCAL_STATIC_BONUS
        elif race_gap >= SAFE_NEUTRAL_MARGIN + 2:
            mult *= FOUR_PLAYER_SAFE_RACE_BONUS
    ...
    def _opening_capture_confidence(self, target: Planet, plan: CapturePlan, mission: str) -> float:
        ...
        if target.owner == -1:
            ...
            if race_gap <= 0:
                ...
                confidence += 0.10
            if self.state.num_players >= 4 and not self.modes.get("is_dominating"):
                if race_gap <= 0:
                    confidence -= 0.14
                elif race_gap == 1:
                    confidence -= 0.08
                if plan.eta >= FOUR_PLAYER_LONG_NEUTRAL_ETA:
                    confidence -= 0.12
                elif is_orbiting_planet(target) and plan.eta >= FOUR_PLAYER_LONG_ORBITING_ETA:
                    confidence -= 0.10
                if self.modes.get("four_player_crowded") and plan.eta >= 12:
                    confidence -= 0.06
                if not is_orbiting_planet(target) and plan.eta <= 10 and race_gap >= SAFE_NEUTRAL_MARGIN + 1:
                    confidence += 0.05
        if not is_orbiting_planet(target):
            confidence += 0.22
        else:
    ...
    def _target_value(self, target: Planet, eta: int, mission: str) -> float:
        ...
        elif is_inner_orbit_planet(target):
            value *= INNER_ORBIT_VALUE_BONUS
        if target.owner not in (-1, self.state.player):
        ...
        if self.state.num_players >= 4 and target.owner == -1:
            value *= self._four_player_neutral_position_multiplier(target, eta)
    ...
    def _build_comet_missions(self, missions: List[MissionOption]) -> None:
        ...
        for target in sorted(comet_targets, key=lambda t: (-t.production, t.ships)):
            ...
            # boost comet urgency when a new spawn wave is imminent
            turns_until_spawn = min(
                (t - self.state.step for t in COMET_SPAWN_TURNS if t > self.state.step),
                default=999,
            )
            if turns_until_spawn <= COMET_SPAWN_URGENCY_WINDOW:
                score *= COMET_SPAWN_SCORE_BOOST
            missions.append(MissionOption(
                score=score, source_ids=[donor.id], target_id=target.id,
                angles=[plan.angle], etas=[plan.eta], ships=[plan.ships],
    ...
    def _build_expand_missions(self, missions: List[MissionOption]) -> None:
        ...
        for target in candidates:
            ...
            for donor in sorted(self._available_my_planets(), key=lambda s: distance_planets(s, target)):
                ...
                if is_inner_orbit_planet(target) and not segment_circle_intersects(
                    (donor.x, donor.y), (target.x, target.y), SUN_CENTER, SUN_RADIUS
                ):
                    score *= PATH_CLEAR_INNER_ORBIT_BOOST
                missions.append(MissionOption(
                    score=score, source_ids=[donor.id], target_id=target.id,
                    angles=[plan.angle], etas=[plan.eta], ships=[plan.ships],
    ...
    def _build_attack_missions(self, missions: List[MissionOption]) -> None:
        ...
        if self.state.num_players >= 4 and not self.modes.get("four_player_pivot_ready") and not recovery_mode:
        ...
        if (
            ...
            if len(safe_neutrals) >= 2:
                return
        if self.state.is_early and not recovery_mode:
            good_neutrals = [p for p in self.state.neutral_planets if p.production >= OPENING_MIN_PRODUCTION]
            if len(good_neutrals) >= 3:
                return
        elif self.state.is_opening and len(self.state.neutral_planets) > 6 and not recovery_mode:
            return
        if self.state.num_players <= 2:
            targets = sorted(
                self.state.enemy_planets,
    ...
    def _mission_can_commit(
        ...
        if mission.mission in ("attack", "snipe", "swarm") and mission.needed > 0:
            ...
            if total_ships < mission.needed * FORCE_CONCENTRATION_MIN_RATIO:
                return False
...
class GenomeDecisionLogic(DecisionLogic):
    ...
    def _opening_expand_sort_key(self, target: Planet, available: Sequence[Planet]) -> Tuple[Any, ...]:
        ...
        if self.genome.opening_range_profile == "eta_focus":
            return (
                ...
                1 if reaction_gap < SAFE_NEUTRAL_MARGIN else 0,
                my_t,
                safe_rank,
                -target.production,
                min_distance,
    ...
    def _opening_range_value_multiplier(self, target: Planet, eta: int) -> float:
        ...
        if self.genome.opening_range_profile == "eta_focus":
            ...
            if orbiting and eta > 8:
                mult *= 0.78
            if reaction_gap < SAFE_NEUTRAL_MARGIN:
                mult *= 0.86
            if my_t >= 12:
                mult *= 0.90
            return mult
    ...
    def _four_player_neutral_position_multiplier(self, target: Planet, eta: int) -> float:
        ...
        if self.genome.position_profile == "safer_neutrals":
            ...
            if race_gap <= 1:
                mult *= 0.92
            if not orbiting and eta <= 10 and race_gap >= SAFE_NEUTRAL_MARGIN + 1:
                mult *= 1.03
            return mult
        ...
        if race_gap <= 1:
            ...
            mult *= 0.94
        if not orbiting and eta <= 9 and race_gap >= SAFE_NEUTRAL_MARGIN + 1:
            mult *= 1.07
        return mult
    ...
    def _opening_capture_confidence(self, target: Planet, plan: Any, mission: str) -> float:
        ...
        if (
            ...
            if self.genome.position_profile == "safer_neutrals":
                ...
                if race_gap <= 1:
                    confidence -= 0.06
                if not is_orbiting_planet(target) and plan.eta <= 9 and race_gap >= SAFE_NEUTRAL_MARGIN + 1:
                    confidence += 0.03
        if self.genome.style_profile == "aggressive":
    ...
    def _build_expand_missions(self, missions: List[MissionOption]) -> None:
        ...
        if self.state.is_opening and available:
            if self._mtmr_opening_active():
                ...
                candidates = [target for target in candidates if target.production >= OPENING_MIN_PRODUCTION]
    ...
    def _build_attack_missions(self, missions: List[MissionOption]) -> None:
        if self.state.num_players > 2:
            ...
            if (
                ...
                safe_neutrals = [
                    target for target in self.state.neutral_planets
                    if target.production >= OPENING_MIN_PRODUCTION and self._is_safe_neutral(target)
                ]
                if len(safe_neutrals) >= 2:
                    return
            if self.state.is_early and not vulture_active:
                good_neutrals = [target for target in self.state.neutral_planets if target.production >= OPENING_MIN_PRODUCTION]
                if len(good_neutrals) >= 3:
                    return
