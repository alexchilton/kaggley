...
INNER_ORBIT_BONUS = 1.08       # modest bonus for weakly-held inner orbit targets
INNER_ORBIT_MAX_SHIPS = 40     # only apply when planet is lightly defended
...
class DecisionLogic:
    ...
    def _target_value(self, target: Planet, eta: int, mission: str) -> float:
        ...
        # Inner orbit bonus: only when planet is lightly defended (uncontested grab)
        if (
