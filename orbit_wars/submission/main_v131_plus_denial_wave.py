"""
Orbit Wars - denial + wave: neutral planet denial combined with synchronized wave attacks
Key changes on top of v131_plus_2p:
- DENIAL: Detect neutrals being targeted by enemy fleets and race to capture them first
- WAVE: Pre-score pass each turn to find which target 3+ planets agree on; boost that
  target so multiple planets fire simultaneously for coordinated arrivals
...
# Wave constants
WAVE_MIN_PLANETS = 3       # how many planets must agree before coordinated launch
WAVE_SCORE_BONUS = 20.0    # score boost for the agreed wave target
...
def agent(obs):
    ...
    # === WAVE ATTACK: pre-score pass to find consensus target ===
    _wave_votes = {}
    for _wsrc in my:
        _win_flight = in_flight_ships_from.get(_wsrc['id'], 0)
        _wavail = _wsrc['ships'] - _win_flight
        if _wavail < 10:
            continue
        _wbest_score = -1e9
        _wbest_id = None
        _wsrc_x, _wsrc_y = _wsrc['x'], _wsrc['y']
        for _wt in (enemy + neutrals):
            _wd = math.hypot(_wt['x'] - _wsrc_x, _wt['y'] - _wsrc_y)
            _wsc = _wt['prod'] * _SCORE_PROD_WEIGHT - _wd * (_SCORE_TT_PENALTY / 5.0)
            if _wt['owner'] == -1:
                _wsc += _NEUTRAL_BONUS
            elif _wt['owner'] != player:
                _wsc += _AGGRO_HOSTILE_BONUS * 0.5
            if _wsc > _wbest_score:
                _wbest_score = _wsc
                _wbest_id = _wt['id']
        if _wbest_id is not None:
            _wave_votes[_wbest_id] = _wave_votes.get(_wbest_id, 0) + 1
    wave_target_id = None
    if _wave_votes:
        _wtop, _wcnt = max(_wave_votes.items(), key=lambda _x: _x[1])
        if _wcnt >= WAVE_MIN_PLANETS:
            wave_target_id = _wtop
    # === END WAVE ATTACK PRE-SCORE ===
    ...
    for src in my:
        ...
        for t in candidates:
            if t['id'] == src['id']:
                continue
            if t['id'] in in_flight_to and t['id'] != wave_target_id:
                continue
            if t['id'] in targeted_this_turn and t['id'] != wave_target_id:
                continue
            ...
            # WAVE: boost score for the consensus wave target
            if t['id'] == wave_target_id:
                score += WAVE_SCORE_BONUS
