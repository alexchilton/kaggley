"""
Orbit Wars - evac: evacuation and ship redistribution
Key changes on top of v131_plus_2p:
- Detect doomed friendly planets (enemy fleet inbound, can't hold) and evacuate ships
- Redistribute surplus ships from safe planets to the frontline
"""
import os
...
# Evacuation / redistribution constants
EVAC_SAFETY_MARGIN = 2.0        # Fixed: require enemy 2× our garrison before evacuating
EVAC_MIN_PLANET_SHIPS = 20      # Fixed: don't evacuate near-empty planets
REDIST_SURPLUS_THRESHOLD = 40
REDIST_MIN_SHIPS = 15
...
def agent(obs):
    ...
    # === EVACUATION: detect doomed planets and evacuate ships ===
    # Build enemy fleet -> my planet threat map
    _enemy_incoming = {}  # my_planet_id -> {'ships': total, 'min_eta': min_eta}
    for _fid, _f in fleets.items():
        ...
        if _best_pid not in _enemy_incoming:
            _enemy_incoming[_best_pid] = {'ships': 0.0, 'min_eta': 999.0}
        _enemy_incoming[_best_pid]['ships'] += _f['ships']
        _enemy_incoming[_best_pid]['min_eta'] = min(_enemy_incoming[_best_pid]['min_eta'], _eta)

    # Identify doomed planets
    doomed_planets = set()
    for _pid, _info in _enemy_incoming.items():
        _p = planets.get(_pid)
        if _p is None:
            continue
        # Don't evacuate near-empty planets — leave them to fight or die
        if _p['ships'] < EVAC_MIN_PLANET_SHIPS:
            continue
        # Skip if a friendly fleet is already reinforcing this planet
        if _pid in in_flight_to:
            continue
        _eta = _info['min_eta']
        # Survivable if my ships + growth during eta >= incoming enemy * margin
        _survivable = _p['ships'] + _p['prod'] * _eta
        if _info['ships'] > _survivable * EVAC_SAFETY_MARGIN:
            doomed_planets.add(_pid)

    # Generate evacuation moves for doomed planets
    for _pid in doomed_planets:
        _p = planets[_pid]
        _in_fl = in_flight_ships_from.get(_pid, 0)
        _al = launched_this_turn.get(_pid, 0)
        _avail = _p['ships'] - _in_fl - _al
        if _avail <= 1:
            continue
        # Find nearest non-doomed friendly planet
        _best_evac = None
        _best_evac_dist = float('inf')
        for _other in my:
            if _other['id'] == _pid or _other['id'] in doomed_planets:
                continue
            _dist = math.hypot(_other['x'] - _p['x'], _other['y'] - _p['y'])
            if _dist < _best_evac_dist:
                _best_evac_dist = _dist
                _best_evac = _other
        if _best_evac:
            _send = max(1, _avail - 1)  # Evacuate all but 1
            _angle = safe_angle(_p['x'], _p['y'], _best_evac['x'], _best_evac['y'])
            moves.append([_pid, _angle, _send])
            launched_this_turn[_pid] = launched_this_turn.get(_pid, 0) + _send
    # === END EVACUATION PRE-PASS ===
    ...
    for src in my:
        ...
        # EVACUATION: skip doomed planets (already evacuated in pre-pass)
    ...
    # === REDISTRIBUTION: move surplus ships from safe back-line planets to frontline ===
    if enemy:
        for _src in my:
            if _src['id'] in doomed_planets:
                continue
            _in_fl = in_flight_ships_from.get(_src['id'], 0)
            _al = launched_this_turn.get(_src['id'], 0)
            _avail = _src['ships'] - _in_fl - _al
            if _avail < REDIST_SURPLUS_THRESHOLD + REDIST_MIN_SHIPS:
                continue
            _surplus = _avail - REDIST_SURPLUS_THRESHOLD
            if _surplus < REDIST_MIN_SHIPS:
                continue
            # Find best frontline friendly planet (close to enemies)
            _best_front = None
            _best_front_score = -1e9
            for _other in my:
                if _other['id'] == _src['id'] or _other['id'] in doomed_planets:
                    continue
                _min_enemy_dist = min(math.hypot(_e['x'] - _other['x'], _e['y'] - _other['y']) for _e in enemy)
                _my_dist = math.hypot(_other['x'] - _src['x'], _other['y'] - _src['y'])
                if _my_dist < 5:
                    continue  # Same planet essentially
                _front_score = -_min_enemy_dist - _my_dist * 0.3
                if _front_score > _best_front_score:
                    _best_front_score = _front_score
                    _best_front = _other
            if _best_front and not path_crosses_sun(_src['x'], _src['y'], _best_front['x'], _best_front['y'], margin=1.5):
                _angle = safe_angle(_src['x'], _src['y'], _best_front['x'], _best_front['y'])
                moves.append([_src['id'], _angle, _surplus])
                launched_this_turn[_src['id']] = launched_this_turn.get(_src['id'], 0) + _surplus
    # === END REDISTRIBUTION ===
