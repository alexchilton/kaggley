...
INTERCEPT_REINFORCE_BASE = 55.0
INTERCEPT_DENY_BASE = 70.0


class DecisionLogicIntercept(DecisionLogic):
    """Shunlite variant: adds high-priority missions to intercept/deny enemy fleets."""

    def _build_intercept_missions(self, missions: List[MissionOption]) -> None:
        for planet_id, arrivals in self.world.arrivals_by_planet.items():
            ...
            if planet is None:
                continue
            enemy_eta = min(eta for eta, _, _ in enemy_arrivals)
            enemy_ships = sum(ships for eta, _, ships in enemy_arrivals if eta <= enemy_eta + 2)

            if planet.owner == self.state.player:
                # Reinforce own planet before enemy arrives
                for donor in sorted(
                    self._available_my_planets(excluded_ids=[planet_id]),
                    key=lambda s: distance_planets(s, planet),
                ):
                    if self.expired():
                        return
                    ...
                    if surplus <= 0:
                        continue
                    shot = self._plan_shot(donor, planet, surplus)
                    if shot is None:
                        continue
                    if shot[1] <= enemy_eta:
                        score = INTERCEPT_REINFORCE_BASE + enemy_ships * 0.3
                        missions.append(MissionOption(
                            score=score, source_ids=[donor.id], target_id=planet_id,
                            angles=[shot[0]], etas=[shot[1]], ships=[surplus],
                            needed=int(enemy_ships * 1.1 + 1), mission="reinforce",
                        ))
                        break
            elif planet.owner == -1:
                # Grab neutral before enemy fleet arrives
                for donor in sorted(
                    self._available_my_planets(),
                    key=lambda s: distance_planets(s, planet),
                ):
                    if self.expired():
                        return
                    ...
                    if surplus <= 0:
                        continue
                    plan = self._settle_plan(donor, planet, surplus, mission="expand")
                    if plan is None:
                        continue
                    if plan.eta <= enemy_eta:
                        score = INTERCEPT_DENY_BASE + planet.production * 6
                        missions.append(MissionOption(
                            score=score, source_ids=[donor.id], target_id=planet_id,
                            angles=[plan.angle], etas=[plan.eta], ships=[plan.ships],
                            needed=plan.required_ships, mission="expand",
    ...
    def _build_all_missions(self) -> List[MissionOption]:
        ...
        self._build_intercept_missions(missions)
        return missions


def agent(obs: Any, config: Any) -> List[List[float | int]]:
    """Intercept variant: generates urgent missions to block incoming enemy fleets."""
    try:
        logic = DecisionLogicIntercept(obs, config)
        return logic.decide()
