...
DENIAL_BOOST = 50.0         # score bonus when enemy fleet is heading to this neutral
DENIAL_URGENCY_SCALE = 30.0 # extra bonus = URGENCY_SCALE / enemy_eta (earlier threat = higher urgency)
DENIAL_HIGH_PROD_BONUS = 20.0
DENIAL_HIGH_PROD_THRESHOLD = 4


class DecisionLogicDenial(DecisionLogic):
    """Shunlite variant: prioritises neutrals that enemy fleets are heading toward."""

    def _build_all_missions(self) -> List[MissionOption]:
        missions = super()._build_all_missions()

        # Map neutral planet_id -> min enemy ETA (from arrival ledger)
        neutral_enemy_eta: Dict[int, float] = {}
        for planet_id, arrivals in self.world.arrivals_by_planet.items():
            planet = self.state.planets_by_id.get(planet_id)
            if planet is None or planet.owner != -1:
                continue
            enemy_arr = [
                (eta, ships) for eta, owner, ships in arrivals
                if owner not in (-1, self.state.player) and ships > 0
            ]
            if enemy_arr:
                neutral_enemy_eta[planet_id] = min(eta for eta, _ in enemy_arr)

        # High-prod neutrals where enemy is geometrically closer than I am
        high_prod_enemy_closer: set = set()
        if self.state.my_planets and self.state.enemy_planets:
            for neutral in self.state.neutral_planets:
                if neutral.production < DENIAL_HIGH_PROD_THRESHOLD:
                    continue
                my_dist = min(distance_planets(neutral, p) for p in self.state.my_planets)
                enemy_dist = min(distance_planets(neutral, e) for e in self.state.enemy_planets)
                if enemy_dist < my_dist:
                    high_prod_enemy_closer.add(neutral.id)

        # Apply bonuses to existing expand/snipe/comet missions
        for m in missions:
            if m.mission not in ("expand", "snipe", "comet"):
                continue
            target = self.state.planets_by_id.get(m.target_id)
            if target is None or target.owner != -1:
                continue
            if m.target_id in neutral_enemy_eta:
                enemy_eta = neutral_enemy_eta[m.target_id]
                m.score += DENIAL_BOOST + DENIAL_URGENCY_SCALE / max(enemy_eta, 1.0)
            elif m.target_id in high_prod_enemy_closer:
                m.score += DENIAL_HIGH_PROD_BONUS

        return missions


def agent(obs: Any, config: Any) -> List[List[float | int]]:
    """Denial variant: urgently grabs neutrals targeted by enemy fleets."""
    try:
        logic = DecisionLogicDenial(obs, config)
        return logic.decide()
