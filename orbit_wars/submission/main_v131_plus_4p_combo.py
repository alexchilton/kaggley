"""
Orbit Wars - 4p combo: political targeting + neutral denial + synchronized wave attacks
Key changes on top of v131_plus_4p:
- POLITICAL: Compute player standings each turn and assign target biases
- DENIAL: Detect neutrals being targeted by enemy fleets and race to capture them first
- WAVE: Pre-score pass to find consensus target; coordinate simultaneous arrivals
...
HIGH_PROD_THRESHOLD = 3        # minimum prod to trigger proactive denial
PROACTIVE_BONUS = 15.0         # score boost for proactive captures

# Wave constants
WAVE_MIN_PLANETS = 3
WAVE_SCORE_BONUS = 20.0
...
def agent(obs):
    ...
    for src in my:
        ...
        for t in candidates:
            ...
            # WAVE: boost score for consensus wave target
