...
DEFENSE_HORIZON = 28
COMET_LOOKAHEAD = 24
COMET_SPAWN_TURNS = frozenset({50, 150, 250, 350, 450})  # fixed comet spawn windows
COMET_URGENCY_WINDOW = 10   # turns either side of spawn to boost comet score
COMET_URGENCY_BOOST = 1.08  # 8% boost when spawn imminent
...
class DecisionLogic:
    ...
    def _build_comet_missions(self, missions: List[MissionOption]) -> None:
        ...
        for target in sorted(comet_targets, key=lambda t: (-t.production, t.ships)):
            ...
            # Urgency boost when a comet spawn is imminent
            if any(abs(self.state.step - st) <= COMET_URGENCY_WINDOW for st in COMET_SPAWN_TURNS):
                score *= COMET_URGENCY_BOOST
            missions.append(MissionOption(
                score=score, source_ids=[donor.id], target_id=target.id,
                angles=[plan.angle], etas=[plan.eta], ships=[plan.ships],
