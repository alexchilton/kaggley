...
class DecisionLogic:
    ...
    def _mission_can_commit(
        ...
        if self._mission_blocks_target(mission) and (
            ...
            return False
        # Force sufficiency check: skip attacks that can't realistically capture
        if mission.mission in ("attack", "swarm", "snipe") and mission.needed > 0:
            total_send = sum(mission.ships)
            if total_send < mission.needed * 0.6:
                # In 4p behind or contested, don't waste ships on hopeless attacks
                modes = getattr(self, "modes", {})
                step = getattr(self.state, "step", 0)
                if step > 30 and (getattr(self.state, "num_players", 2) >= 4 or modes.get("is_behind")):
                    return False
    ...
    _RL_POLICY_WEIGHTS_2P = [
        7.051664507845469e-17, 2.4808502730843523e-15, 2.4808502730843523e-15,
        0.0, 8.992806499463643e-16, 7.278737626371766e-20,
        1.0436096431476316e-15, 7.549516567451067e-17, -8.709660676004925e-18,
        7.844938380393715e-17, 1.4594866590074564e-15, 8.866773487412292,
        3.1763637559791165, -0.3448028273369212, -1.076285405431472,
        -0.18510892590936748, -1.3539654516294002, 2.8485267495375055,
        0.29646751496136353, 0.024159097753365322, 0.01936314673185087,
        -0.043522244485216126, 0.7346151878742125, 0.10639519087436337,
        0.34509881848232593, 0.22823107747610347, -0.7152520411423636,
        0.0, -0.0822360931209984, -0.6168521404436427, 0.0034853664117344366,
        -5.518852157582609e-20, -4.456771087104229,
    ]
    _RL_POLICY_WEIGHTS_4P = [
        3.040114815769119e-17, 2.4295870849839603e-15, 1.643130076445233e-16,
        1.5586281074033028e-15, 9.237055564881159e-16, 9.49048581549973e-16,
        8.570921750106081e-16, 1.376676550535195e-16, -3.8310512402659914e-17,
        7.353669354223071e-17, 1.257918990514084e-15, 8.607510868320071,
        3.6384430679639026, -1.249180893924929, -1.198777182355406,
        -0.15979575828126671, -0.9745001091107924, 2.075119308937274,
        0.13805861282715984, 0.46033325696982486, 0.2676407167887956,
        -0.7279739737586224, 0.1807302237109644, 0.4844134642142364,
        -0.08542320816754964, 1.0603863602631287, -0.41568595223270216,
        0.00039257934343783513, 3.7747582837255325e-17, -1.2248134671315152,
        -0.19435217583101963, -1.522788767670114e-19, -4.539595679451899,
    ]
    ...
    def _rl_window_open(self) -> bool:
        ...
        if self.state.num_players <= 2:
            ...
            if ship_share < 0.28 or ship_share > 0.78:
                return False
        else:
