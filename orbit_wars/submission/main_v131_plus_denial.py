"""
Orbit Wars - denial: neutral planet denial
Key changes on top of v131_plus_2p:
- Detect neutrals being targeted by enemy fleets and race to capture them first
- Proactive denial bonus for high-production neutrals near enemies
"""
import os
...
def optimal_intercept_time(fx: float, fy: float, tx: float, ty: float,
                           omega: float, ships: int, max_delay: int = 25):
    """For an orbiting planet currently at (tx, ty), find the launch delay in [0, max_delay]
    that minimises (delay + travel_time_to_intercept).  Returns (best_delay, best_arrival).
    For non-orbiting targets just pass omega=0 and it returns (0, current_tt)."""
    if omega == 0:
        _, _, tt = solve_intercept(fx, fy, tx, ty, False, 0, ships)
        return 0, tt
    ...
    for delay in range(max_delay + 1):
        if delay == 0:
            ...
            px, py = predict_orbit(tx, ty, omega, float(delay))
        ...
        if arrival < best_arrival:
            ...
            if delay >= 5 and worse_streak >= 4:
                break  # Clearly past the minimum, stop early
        prev_arrival = arrival
    return best_delay, best_arrival


# Sniper-nest constants — small neutral planets within this distance of our big planets
SNIPER_RANGE = float(os.environ.get('V131_SNIPER_RANGE', '35'))
SNIPER_MIN_MY_SHIPS = int(os.environ.get('V131_SNIPER_MIN_MY_SHIPS', '40'))
SNIPER_BONUS = float(os.environ.get('V131_SNIPER_BONUS', '60.0'))
...
def planet_under_threat(p_id, fleets, planets, player, omega, planet_nearest, fleet_target_map):
    # Count enemy ships in fleets whose nearest planet is p_id (proxy for "heading here").
    # Previously used fleet_target_map (only contains our fleet IDs) → always returned 0.
    incoming = 0
    p = planets[p_id]
    p_x, p_y = p['x'], p['y']
    ...
    for f in fleets.values():
        if f['owner'] == player:
            continue
        tgt_id = planet_nearest.get(f['id'])
        if tgt_id != p_id:
            continue
        # Only treat as threat if within ~80 turns' travel
        f_x, f_y = f['x'], f['y']
        d = math.sqrt((f_x - p_x) ** 2 + (f_y - p_y) ** 2)
        if d > 400:
            continue
        incoming += f['ships']
    return incoming
...
def agent(obs):
    ...
    # === DENIAL: build enemy fleet -> neutral target map ===
    ...
    for _fid, _f in fleets.items():
        ...
        _dbonus = DENIAL_BASE_SCORE + _tp['prod'] * DENIAL_PROD_WEIGHT + (1.0 / max(_eta, 0.5)) * DENIAL_URGENCY_WEIGHT
        if _best_pid not in _denial_bonus or _dbonus > _denial_bonus[_best_pid][0]:
            _denial_bonus[_best_pid] = (_dbonus, _eta)

    # Proactive denial: high-prod neutrals enemy can reach within 20 turns
    for _tp in neutrals:
        if _tp['prod'] < HIGH_PROD_THRESHOLD:
            continue
        if _tp['id'] in _denial_bonus:
            continue  # Already tracked reactively
        if not enemy:
            continue
        ...
        if _min_my_tt < _min_enemy_tt:  # I'm closer than the enemy
            _denial_bonus[_tp['id']] = (PROACTIVE_BONUS, _min_enemy_tt)

    # === SNIPER NEST DENIAL: neutral planets very close to our large planets ===
    # If enemy captures a small nearby neutral, they get a close-range launch pad
    # to snipe our big planet. Boost denial priority for these.
    if my:
        _big_planets = [_p for _p in my if _p['ships'] >= SNIPER_MIN_MY_SHIPS]
        for _tp in neutrals:
            for _bp in _big_planets:
                _sd = math.sqrt((_tp['x'] - _bp['x']) ** 2 + (_tp['y'] - _bp['y']) ** 2)
                if _sd <= SNIPER_RANGE:
                    _sniper_bonus = SNIPER_BONUS + _tp['prod'] * DENIAL_PROD_WEIGHT
                    if _tp['id'] not in _denial_bonus or _sniper_bonus > _denial_bonus[_tp['id']][0]:
                        _my_tt = travel_time(_bp['x'], _bp['y'], _tp['x'], _tp['y'], 20)
                        _denial_bonus[_tp['id']] = (_sniper_bonus, _my_tt)
                    break  # Already boosted this neutral — no need to check other big planets
    # === END DENIAL PRE-PASS ===

    # === EVACUATION: save ships from planets we're about to lose ===
    # Only run outside decisive winning phases so we don't waste time on it.
    _evac_skip_phases = {'smash', 'cleanup', 'pressure', 'crush'}
    if phase not in _evac_skip_phases:
        for _ep in my:
            _ep_id = _ep['id']
            _ep_threat = threats.get(_ep_id, 0)
            if _ep_threat <= 0:
                continue
            # Estimate ETA of the closest threatening fleet
            _min_eta = float('inf')
            for _f in fleets.values():
                if _f['owner'] == player:
                    continue
                if fleet_nearest_planet.get(_f['id']) != _ep_id:
                    continue
                _f_eta = travel_time(_f['x'], _f['y'], _ep['x'], _ep['y'], int(max(_f['ships'], 1)))
                if _f_eta < _min_eta:
                    _min_eta = _f_eta
            if _min_eta == float('inf'):
                continue
            # Projected garrison at time of arrival
            _in_flt = in_flight_ships_from.get(_ep_id, 0)
            _launched = launched_this_turn.get(_ep_id, 0)
            _projected_garrison = _ep['ships'] - _in_flt - _launched + _ep['prod'] * max(1, int(_min_eta))
            if _projected_garrison > _ep_threat * 1.05:
                continue  # We can hold — no evacuation needed
            # Planet is doomed — evacuate 70% of available ships to the nearest friendly
            _avail = int(_ep['ships'] - _in_flt - _launched)
            _evac_ships = int(_avail * 0.70)
            if _evac_ships < 5:
                continue
            # Find nearest friendly destination (not self)
            _best_dst = None
            _best_dst_d = float('inf')
            for _dst in my:
                if _dst['id'] == _ep_id:
                    continue
                _dd = math.sqrt((_dst['x'] - _ep['x']) ** 2 + (_dst['y'] - _ep['y']) ** 2)
                if _dd < _best_dst_d:
                    _best_dst_d = _dd
                    _best_dst = _dst
            if _best_dst is None:
                continue
            _ix, _iy, _tt = solve_intercept(_ep['x'], _ep['y'], _best_dst['x'], _best_dst['y'],
                                             _best_dst['is_orb'], omega, _evac_ships)
            if path_crosses_sun(_ep['x'], _ep['y'], _ix, _iy, margin=1.5):
                continue
            _angle = safe_angle(_ep['x'], _ep['y'], _ix, _iy)
            moves.append([_ep_id, _angle, _evac_ships])
            launched_this_turn[_ep_id] = launched_this_turn.get(_ep_id, 0) + _evac_ships
    # === END EVACUATION ===
    ...
    for src in my:
        ...
        if phase == 'expand':
            ...
            for t in neutrals:
                ...
                prod_w = 8.0 if step < 80 else 3.5
                dist_w = 1.5 if step < 80 else 3.0
                score = -d * dist_w + t['prod'] * prod_w
        ...
        for t in candidates:
            ...
            # DENIAL: boost score for neutral planets being raced by enemy fleets
            if t['owner'] == -1 and t['id'] in _denial_bonus:
                _dbonus, _enemy_eta = _denial_bonus[t['id']]
                if tt <= _enemy_eta:  # Only if we can actually get there in time
                    score += _dbonus
