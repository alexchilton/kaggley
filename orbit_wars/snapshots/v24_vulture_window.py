...
FOUR_PLAYER_VULTURE_ETA_LIMIT = 14
FOUR_PLAYER_VULTURE_MIN_INCOMING = 8
FOUR_PLAYER_VULTURE_HOSTILE_BONUS = 1.24
FOUR_PLAYER_VULTURE_ATTACK_CAP = 5
...
class DecisionLogic:
    ...
    def _planet_under_third_party_pressure(self, target: Planet) -> bool:
        ...
        incoming = sum(
            ...
            and eta <= FOUR_PLAYER_VULTURE_ETA_LIMIT
        )
        return incoming >= FOUR_PLAYER_VULTURE_MIN_INCOMING
    ...
    def _build_modes(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "vulture_owner_ids": vulture_owner_ids,
            "vulture_active": bool(vulture_owner_ids),
        }
    ...
    def _turn_launch_cap(self) -> int:
        ...
        if num_players >= 4 and not modes.get("is_cleanup"):
            if modes.get("vulture_active"):
                return min(MAX_TURN_LAUNCHES, FOUR_PLAYER_VULTURE_ATTACK_CAP)
    ...
    def _target_value(self, target: Planet, eta: int, mission: str) -> float:
        ...
        if self.state.num_players >= 4 and target.owner not in (-1, self.state.player):
            ...
            if (
                target.owner in modes.get("vulture_owner_ids", [])
                or self._planet_under_third_party_pressure(target)
            ):
                value *= FOUR_PLAYER_VULTURE_HOSTILE_BONUS
    ...
    def _build_attack_missions(self, missions: List[MissionOption]) -> None:
        ...
        if (
            self.state.num_players >= 4
            and not self.modes.get("vulture_active")
        ...
        if self.state.is_early:
            good_neutrals = [p for p in self.state.neutral_planets if p.production >= OPENING_MIN_PRODUCTION]
            if len(good_neutrals) >= 3 and not self.modes.get("vulture_active"):
                return
        elif self.state.is_opening and len(self.state.neutral_planets) > 6 and not self.modes.get("vulture_active"):
            return
        if self.state.num_players >= 4 and self.modes.get("vulture_active"):
            targets = sorted(
                self.state.enemy_planets,
                key=lambda t: (
                    not self._planet_under_third_party_pressure(t),
                    t.owner not in self.modes.get("vulture_owner_ids", []),
                    t.ships / max(1, t.production),
                    t.ships,
                ),
            )
