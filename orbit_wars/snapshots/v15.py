...
class _LookaheadMixin:
    ...
    def _mission_can_commit(
        ...
        if existing_moves + len(mission.source_ids) > MAX_TURN_LAUNCHES:
            return False
    ...
    def _commit_missions(self, missions: List[MissionOption]) -> List[PlannedMove]:
        ...
        while remaining and not self.expired() and len(moves) < MAX_TURN_LAUNCHES:
...
class _SwarmSyncMixin:
    def _swarm_probe_choices(self, donor: Planet, target: Planet, surplus: int) -> List[Tuple[int, float, int]]:
        values = {
            max(4, surplus // 3),
            max(5, surplus // 2),
            max(1, int(target.ships) + 1),
            max(1, int(target.ships) + int(target.production) + 1),
            surplus,
        }
        choices: List[Tuple[int, float, int]] = []
        for ships in sorted(v for v in values if 1 <= v <= surplus):
            shot = self._plan_shot(donor, target, ships)
            if shot is not None:
                choices.append((ships, shot[0], shot[1]))
        return choices
    ...
    def _build_swarm_missions(self, missions: List[MissionOption]) -> None:
        ...
        for target in targets:
            if self.expired():
                return
            nearby = sorted(available, key=lambda s: distance_planets(s, target))[:MULTI_SOURCE_TOP_K]
            donor_options: List[Tuple[Planet, List[Tuple[int, float, int]]]] = []
            for donor in nearby:
                surplus = self._planet_surplus(donor)
                if surplus <= 0:
                    continue
                choices = self._swarm_probe_choices(donor, target, surplus)
                if choices:
                    donor_options.append((donor, choices))
            if len(donor_options) < 2:
                continue
            for anchor_donor, anchor_choices in donor_options:
                if self.expired():
                    return
                for anchor_ships, anchor_angle, anchor_eta in sorted(anchor_choices, key=lambda item: (-item[2], -item[0])):
                    group = [(anchor_donor, anchor_ships, anchor_angle, anchor_eta)]
                    for donor, choices in donor_options:
                        if donor.id == anchor_donor.id:
                            continue
                        best = None
                        best_key = None
                        for ships, angle, eta in choices:
                            delta = abs(eta - anchor_eta)
                            if delta > 2:
                                continue
                            key = (delta, -ships)
                            if best_key is None or key < best_key:
                                best_key = key
                                best = (ships, angle, eta)
                        if best is not None:
                            group.append((donor, best[0], best[1], best[2]))
                        if len(group) == 3:
                            break
                    if len(group) < 2:
                        continue
                    etas = [item[3] for item in group]
                    if max(etas) - min(etas) > SWARM_ETA_TOLERANCE:
                        continue
...
# Variant patch: full combo + tuned constants.
MAX_TURN_LAUNCHES = 6
SWARM_ETA_TOLERANCE = 2
REACTION_TOP_K = 4
ATTACK_COST_WEIGHT = 0.72
SNIPE_COST_WEIGHT = 1.05
HOSTILE_VALUE_MULT = 1.98
SAFE_NEUTRAL_VALUE_MULT = 1.22
SWARM_VALUE_MULT = 1.10
FINISHING_SEND_BONUS = 5
class DecisionLogicV15(_LookaheadMixin, _SwarmSyncMixin, _OpponentPressureMixin, _ThreatTargetMixin, _DynamicModesMixin, DecisionLogic):
    def decide(self) -> List[List[float | int]]:
        try:
            self.reaction_map = self._build_reaction_map()
...
DecisionLogic = DecisionLogicV15
