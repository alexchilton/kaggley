...
DUEL_SPARSE_LAUNCH_CAP = 3
DUEL_SPARSE_TURN_LIMIT = 20
DUEL_SPARSE_MIN_PRODUCTION = 3
...
OWN_ROUTE_COOLDOWN = 12
OWN_ROUTE_URGENT_FALL_TURN = 8
...
def reset_memory_if_needed(player: int, step: int) -> None:
    ...
    if step <= 0 or AGENT_MEMORY.get("player") != player:
        ...
        AGENT_MEMORY["recent_own_routes"] = {}
...
class DecisionLogic:
    ...
    def _finalize(self, moves: Sequence[PlannedMove]) -> List[List[float | int]]:
        ...
        recent_routes = AGENT_MEMORY.setdefault("recent_own_routes", {})
        ...
        for move in moves:
            ...
            if ships <= 0 or ships > current_by_id[move.source_id].ships:
                continue
            refreshed = self._refresh_move(move, current_by_id[move.source_id])
            if refreshed is None:
                continue
            angle, _eta, target = refreshed
            if target.owner == self.state.player and not self._own_transfer_route_allowed(
                move.source_id, target.id, move.mission
            ):
                continue
            sanitized.append([move.source_id, float(angle), ships])
            used_sources.add(move.source_id)
            if target.owner == self.state.player:
                recent_routes[(move.source_id, target.id)] = self.state.step
        return sanitized
    ...
    def _duel_sparse_opening_active(self) -> bool:
        return (
            getattr(self.state, "num_players", 2) <= 2
            and getattr(self.state, "is_opening", False)
            and getattr(self.state, "step", 0) < DUEL_SPARSE_TURN_LIMIT
            and not self._duel_opening_pivot_ready()
        )

    def _own_transfer_route_allowed(
        self,
        source_id: int,
        target_id: int,
        mission: str,
    ) -> bool:
        target = self.state.planets_by_id.get(target_id)
        if target is None or target.owner != self.state.player:
            return True
        if mission in {"defend", "evacuate"}:
            return True
        recent_routes = AGENT_MEMORY.get("recent_own_routes", {})
        last_step = max(
            recent_routes.get((source_id, target_id), -10**9),
            recent_routes.get((target_id, source_id), -10**9),
        )
        if self.state.step - last_step > OWN_ROUTE_COOLDOWN:
            return True
        hold = self.world.hold_status(
            target_id,
            DEFENSE_HORIZON,
            planned_commitments=self.planned_commitments,
        )
        fall_turn = hold.get("fall_turn")
        return (
            not hold.get("holds_full", True)
            or (fall_turn is not None and int(fall_turn) <= OWN_ROUTE_URGENT_FALL_TURN)
        )

    def _refresh_move(self, move: PlannedMove, source: Planet) -> Optional[Tuple[float, int, Planet]]:
        target = self.state.planets_by_id.get(move.target_id)
        if target is None:
            return None
        shot = self._plan_shot(source, target, int(move.ships))
        if shot is None:
            return None
        angle, eta = shot
        speed = fleet_speed(int(move.ships), max_speed=self.state.max_speed)
        if self.intercept_solver.check_sun_collision(source.x, source.y, angle, speed, eta):
            return None
        if abs(int(eta) - int(move.eta)) > 3:
            return None
        return angle, eta, target
    ...
    def _turn_launch_cap(self) -> int:
        ...
        if self._duel_sparse_opening_active():
            return min(MAX_TURN_LAUNCHES, DUEL_SPARSE_LAUNCH_CAP)
    ...
    def _opening_capture_allowed(self, target: Planet, plan: CapturePlan, mission: str) -> bool:
        ...
        if (
            ...
            return False
        if (
            self._duel_sparse_opening_active()
            and mission == "expand"
            and target.owner == -1
        ):
            if target.production < DUEL_SPARSE_MIN_PRODUCTION and not self._is_safe_neutral(target):
                return False
            if is_orbiting_planet(target) and target.production < 4 and plan.eta > DUEL_SAFE_ROTATING_ETA_LIMIT:
                return False
    ...
    def _build_expand_missions(self, missions: List[MissionOption]) -> None:
        ...
        if self._duel_sparse_opening_active():
            candidates = [
                ...
                if p.production >= DUEL_SPARSE_MIN_PRODUCTION or self._is_safe_neutral(p)
            ]
