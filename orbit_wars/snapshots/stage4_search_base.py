...
DUEL_FINISHING_THRESHOLD = 0.28
DUEL_FINISHING_PROD_RATIO = 1.15
DUEL_FINISHING_STEP = 88
DUEL_FRONTLINE_DISTANCE = 22.0
DUEL_AHEAD_RESERVE_RATIO = 0.10
DUEL_DOMINATING_RESERVE_RATIO = 0.18
DUEL_PROTECT_LAUNCH_CAP = 5
DUEL_PROTECT_SAFE_NEUTRAL_DAMP = 0.94
DUEL_PROTECT_CONTESTED_NEUTRAL_DAMP = 0.88
DUEL_PROTECT_HOSTILE_BONUS = 1.04
DUEL_FINISH_HOSTILE_BONUS = 1.08
...
FOUR_PLAYER_DOGPILE_TURN_LIMIT = 96
FOUR_PLAYER_DOGPILE_LAUNCH_CAP = 3
FOUR_PLAYER_DOGPILE_SAFE_NEUTRAL_LIMIT = 1
FOUR_PLAYER_DOGPILE_FRONT_RESERVE_RATIO = 0.30
FOUR_PLAYER_DOGPILE_DOUBLE_FRONT_RESERVE_RATIO = 0.42
FOUR_PLAYER_COUNTERPUNCH_HOSTILE_BONUS = 1.14
FOUR_PLAYER_ORBITING_COUNTERPOST_BONUS = 1.10
FOUR_PLAYER_ORBITING_COUNTERPOST_DISTANCE = 18.0
FOUR_PLAYER_ORBITING_COUNTERPOST_MIN_PRODUCTION = 3
...
class DecisionLogic:
    ...
    def _frontline_enemy_owners(self, planet: Planet) -> set[int]:
        if self.state.num_players < 4:
            return set()
        return {
            int(enemy.owner)
    ...
    def _frontline_reserve(self, planet: Planet) -> int:
        ...
        if self.modes.get("four_player_dogpile_mode"):
            if nearby >= 2:
                return int(planet.ships * FOUR_PLAYER_DOGPILE_DOUBLE_FRONT_RESERVE_RATIO)
            if nearby >= 1:
                return int(planet.ships * FOUR_PLAYER_DOGPILE_FRONT_RESERVE_RATIO)
        if nearby >= 2:
    ...
    def _duel_frontline_reserve(self, planet: Planet) -> int:
        if self.state.num_players > 2 or not self.state.enemy_planets:
            return 0
        if not (self.modes.get("is_ahead") or self.modes.get("is_dominating")):
            return 0
        nearest_enemy = min(distance_planets(planet, enemy) for enemy in self.state.enemy_planets)
        if nearest_enemy > DUEL_FRONTLINE_DISTANCE:
            return 0
        ratio = DUEL_DOMINATING_RESERVE_RATIO if self.modes.get("is_dominating") else DUEL_AHEAD_RESERVE_RATIO
        return int(planet.ships * ratio)
    ...
    def _four_player_pressure_owners(self) -> List[int]:
        if self.state.num_players < 4:
            return []
        owners: set[int] = set()
        for planet in self.state.my_planets:
            owners.update(self._frontline_enemy_owners(planet))
        return sorted(owners)

    def _four_player_backline_donor_key(self, donor: Planet, target: Planet) -> Tuple[int, float, int]:
        return (
            self._frontline_enemy_count(donor),
            distance_planets(donor, target),
            -int(donor.ships),
        )

    def _orbiting_counterpost_candidate(self, target: Planet) -> bool:
        if (
            self.state.num_players < 4
            or target.owner != -1
            or not is_orbiting_planet(target)
            or target.production < FOUR_PLAYER_ORBITING_COUNTERPOST_MIN_PRODUCTION
            or not self._is_safe_neutral(target)
        ):
            return False
        nearest_enemy = min(
            (distance_planets(target, enemy) for enemy in self.state.enemy_planets),
            default=10**9,
        )
        return nearest_enemy <= FOUR_PLAYER_ORBITING_COUNTERPOST_DISTANCE
    ...
    def _planet_surplus(self, planet: Planet, turns_ahead: int = DEFENSE_HORIZON) -> int:
        ...
        reserve = max(
            ...
            self._duel_frontline_reserve(effective),
    ...
    def _build_modes(self) -> Dict[str, Any]:
        ...
        if self.state.num_players <= 2:
            ...
            is_finishing = (
                domination > DUEL_FINISHING_THRESHOLD
                and my_prod > max_enemy_prod * DUEL_FINISHING_PROD_RATIO
                and self.state.step > DUEL_FINISHING_STEP
            )
        else:
        ...
        four_player_dogpile_mode = False
        pressure_owner_ids: List[int] = []
        ...
        if self.state.num_players >= 4:
            ...
            pressure_owner_ids = self._four_player_pressure_owners()
            if (
            ...
            four_player_dogpile_mode = (
                self.state.step <= FOUR_PLAYER_DOGPILE_TURN_LIMIT
                and len(pressure_owner_ids) >= 2
                and not is_ahead
        ...
        return {
            ...
            "four_player_dogpile_mode": four_player_dogpile_mode,
            "pressure_owner_ids": pressure_owner_ids,
    ...
    def _turn_launch_cap(self) -> int:
        ...
        if num_players >= 4 and not modes.get("is_cleanup"):
            if modes.get("four_player_dogpile_mode"):
                return min(MAX_TURN_LAUNCHES, FOUR_PLAYER_DOGPILE_LAUNCH_CAP)
        ...
        if (
            num_players <= 2
            and (modes.get("is_ahead") or modes.get("is_dominating"))
            and not modes.get("is_finishing")
        ):
            return min(MAX_TURN_LAUNCHES, DUEL_PROTECT_LAUNCH_CAP)
        return MAX_TURN_LAUNCHES
    ...
    def _target_value(self, target: Planet, eta: int, mission: str) -> float:
        ...
        if (
            ...
            value *= 0.95
        if self.state.num_players <= 2 and (modes.get("is_ahead") or modes.get("is_dominating")):
            if target.owner == -1:
                if self._is_contested_neutral(target):
                    value *= DUEL_PROTECT_CONTESTED_NEUTRAL_DAMP
                elif self._is_safe_neutral(target):
                    value *= DUEL_PROTECT_SAFE_NEUTRAL_DAMP
            elif target.owner != self.state.player:
                if modes.get("is_finishing") or self.state.is_late:
                    value *= DUEL_FINISH_HOSTILE_BONUS
                else:
                    value *= DUEL_PROTECT_HOSTILE_BONUS
        ...
        if self.state.num_players >= 4 and target.owner not in (-1, self.state.player):
            ...
            if target.owner == modes.get("runaway_owner"):
                ...
                value *= 0.94
            if target.owner in set(modes.get("pressure_owner_ids", [])) and (
                modes.get("four_player_dogpile_mode") or modes.get("four_player_recovery_mode")
            ):
                value *= FOUR_PLAYER_COUNTERPUNCH_HOSTILE_BONUS
            if modes.get("four_player_pivot_ready"):
        ...
        if (
            ...
            if self._orbiting_counterpost_candidate(target):
                value *= FOUR_PLAYER_ORBITING_COUNTERPOST_BONUS
    ...
    def _build_expand_missions(self, missions: List[MissionOption]) -> None:
        ...
        if not candidates:
            return
        dogpile_mode = self.state.num_players >= 4 and self.modes.get("four_player_dogpile_mode")
            ...
            if available:
                ...
                if dogpile_mode:
                    candidates = [
                        ...
                        if (
                            not is_orbiting_planet(p)
                            or self._orbiting_counterpost_candidate(p)
                        )
                    ]
                    candidates.sort(key=lambda t: (
                        0 if not is_orbiting_planet(t) else 1,
                        0 if self._is_safe_neutral(t) else 1,
                        0 if self._orbiting_counterpost_candidate(t) else 1,
                        -t.production,
                        min(distance_planets(t, s) for s in available),
                        t.ships / max(1, t.production),
                    ))
        ...
        for target in candidates:
            if self.expired():
                return
            donor_key = (
                (lambda s: self._four_player_backline_donor_key(s, target))
                if dogpile_mode
                else (lambda s: distance_planets(s, target))
            )
            for donor in sorted(self._available_my_planets(), key=donor_key):
                surplus = self._planet_surplus(donor)
                if surplus <= 0:
                    continue
                ...
                if not self._candidate_time_valid(target, plan.eta):
                    continue
                if dogpile_mode and is_orbiting_planet(target) and not self._orbiting_counterpost_candidate(target):
                    continue
    ...
    def _build_attack_missions(self, missions: List[MissionOption]) -> None:
        ...
        if (
            ...
            if len(safe_neutrals) >= 2:
                return
        if self.state.is_early and not recovery_mode and not dogpile_mode:
            good_neutrals = [p for p in self.state.neutral_planets if p.production >= OPENING_MIN_PRODUCTION]
            if len(good_neutrals) >= 3:
                return
        elif self.state.is_opening and len(self.state.neutral_planets) > 6 and not recovery_mode and not dogpile_mode:
            return
        if self.state.num_players <= 2:
            targets = sorted(
                self.state.enemy_planets,
            ...
            pressure_owner_ids = set(self.modes.get("pressure_owner_ids", []))
            targets = sorted(
                self.state.enemy_planets,
                key=lambda t: (
                    t.owner not in pressure_owner_ids,
                    min(distance_planets(t, s) for s in self.state.my_planets),
                    t.ships / max(1, t.production),
                    -t.production,
                    t.ships,
        ...
        for target in targets:
            ...
            donor_key = (
                (lambda s: self._four_player_backline_donor_key(s, target))
                if (dogpile_mode or recovery_mode)
                else (lambda s: distance_planets(s, target))
            )
            for donor in sorted(self._available_my_planets(), key=donor_key):
                surplus = self._planet_surplus(donor)
    ...
    def _build_reinforce_missions(self, missions: List[MissionOption]) -> None:
        ...
        for planet in self.state.my_planets:
            ...
            for donor in sorted(
                ...
                value *= REINFORCE_VALUE_MULT
                value *= proximity * 0.5
                if dogpile_mode and self._frontline_enemy_count(planet) >= 1 and not is_orbiting_planet(planet):
                    value *= 1.30
