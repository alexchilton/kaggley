    ...
    def _build_modes(self) -> Dict[str, Any]:
        ...
        if self.state.num_players <= 2:
            ...
            mtmr_hostility_ready = (
                ...
                and (
                    mtmr_safe_neutral_count <= MTMR_SAFE_NEUTRAL_LIMIT
                    or my_prod >= max(1, max_enemy_prod) * MTMR_MULTI_HOSTILITY_PROD_RATIO
                    or is_ahead
                    or is_cleanup
                )
            ) or (
                self.state.step >= MTMR_RELAXED_HOSTILITY_TURN
                and mtmr_safe_neutral_count == 0
            )
                ...
                if self._mtmr_opening_active():
                    ...
                    candidates.sort(key=lambda t: (
                        0 if self._mtmr_neutral_allowed(
                            t,
                            CapturePlan(target=t, ships=max(1, t.ships + 1), angle=0.0, eta=self.reaction_map.get(t.id, (10**9, 10**9))[0], eval_turn=self.reaction_map.get(t.id, (10**9, 10**9))[0], required_ships=max(1, t.ships + 1)),
                        ) else 1,
