#!/usr/bin/env python3
"""4p test with ONLY SEND_DOMINATE=0.65 + smash fix (CURG at 1.08 default)."""
...
print(f"4P TEST: v131-plus-4p (v2d) — SEND_DOMINATE=0.65 + smash fix only")
print(f"CURG_MULT=1.08, NEUTRAL_DAMP=0.78 (both at original defaults)")
...
print(f"\n4P RESULT: v131-plus v2d 1st-rate = {wins['v131+']}/{n_games} = {first_rate:.0%}")
print(f"  Breakdown: v131+={wins['v131+']} v131={wins['v131']} shunA={wins['shunA']} shunB={wins['shunB']}")

# Also show seat analysis
print(f"\n  Seat analysis:")
for seat in range(4):
    games_at_seat = [g for g in range(n_games) if (0 + g) % 4 == seat]
    print(f"    Seat {seat}: {len(games_at_seat)} games")
